"""
emailtemp.org 临时邮箱渠道（Laravel + CSRF，与 tempmailten.com 共享模板）
...
def _post_messages(csrf: str, cookie_hdr: str) -> dict:
    ...
    if not isinstance(data, dict):
        raise RuntimeError("emailtemp-org: 解析响应 JSON 失败")
    return data
...
def generate_email(channel: str = CHANNEL) -> EmailInfo:
    ...
    if not m:
        raise RuntimeError("emailtemp-org: 未能从首页提取 CSRF token")
    csrf = m.group(1)

    data = _post_messages(csrf, cookie_hdr)
    mailbox = str(data.get("mailbox") or "").strip()
    if not mailbox or "@" not in mailbox:
        raise RuntimeError(f"emailtemp-org: 邮箱地址无效: {mailbox!r}")
...
def get_emails(email: str, token: str) -> List[Email]:
    ...
    if not email:
        raise ValueError("emailtemp-org: 邮箱地址为空")
