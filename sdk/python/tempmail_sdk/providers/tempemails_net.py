...
创建邮箱: GET / 获取 session 和 CSRF → POST /get_messages 获取自动分配的邮箱地址
获取邮件: POST /get_messages → messages 数组 + GET /view/{id} 获取邮件正文
域名: @tempemails.net
"""
...
def _decode_token(token: str) -> tuple:
    ...
    try:
        ...
        raise ValueError("tempemails-net: token 格式无效") from e
    csrf = data.get("csrf", "")
    cookies = data.get("cookies", {})
    if not csrf or not isinstance(cookies, dict):
        raise ValueError("tempemails-net: token 数据不完整")
    return csrf, cookies
...
def generate_email() -> EmailInfo:
    ...
    流程:
    1. GET / 获取首页 HTML，保存 session cookie（XSRF-TOKEN、temp_mail_session 等）
    2. 从 HTML 中提取 CSRF token: <meta name="csrf-token" content="xxx">
    3. POST /get_messages 获取自动分配的邮箱地址
    ...
    if not m:
        raise RuntimeError("tempemails-net: 无法从首页提取 CSRF token")
    csrf = m.group(1)
    ...
    if not data.get("status"):
        raise RuntimeError(f"tempemails-net: 获取邮箱失败: {data!r}")

    mailbox = (data.get("mailbox") or "").strip()
    if not mailbox or "@" not in mailbox:
        raise RuntimeError(f"tempemails-net: 返回的邮箱地址无效: {data!r}")
...
def get_emails(email: str, token: str) -> List[Email]:
    ...
    2. POST /get_messages 获取邮件列表（messages 数组）
    3. 对每封邮件，GET /view/{id} 获取 HTML 正文
    4. 使用 normalize_email 标准化返回结果
    """
    if not token:
        raise ValueError("tempemails-net: token 不能为空")

    addr = (email or "").strip()
    if not addr:
        raise ValueError("tempemails-net: 邮箱地址不能为空")
