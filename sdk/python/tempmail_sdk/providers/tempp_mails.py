"""
tempp-mails.com 临时邮箱渠道（Laravel + CSRF，与 tempmailten.com 共享模板）
...
def _decode_token(token: str) -> Tuple[str, str]:
    ...
    if not csrf:
        raise ValueError("tempp-mails: token 中缺少 CSRF")
...
def _post_messages(csrf: str, cookie_hdr: str) -> dict:
    ...
    if not isinstance(data, dict):
        raise RuntimeError("tempp-mails: 解析响应 JSON 失败")
    return data
...
def generate_email(channel: str = CHANNEL) -> EmailInfo:
    ...
    if not m:
        raise RuntimeError("tempp-mails: 未能从首页提取 CSRF token")
    csrf = m.group(1)
...
def get_emails(email: str, token: str) -> List[Email]:
    ...
    if not email:
        raise ValueError("tempp-mails: 邮箱地址为空")
