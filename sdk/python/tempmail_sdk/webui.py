...
通过环境变量 ``TEMPMAIL_WEBUI=true`` 激活（默认不启动），可选 ``TEMPMAIL_WEBUI_PORT``
指定端口（未设置则由系统随机分配），可选 ``TEMPMAIL_WEBUI_HOST`` 指定监听主机
（未设置则默认 127.0.0.1）。仅依赖 Python 标准库（http.server + threading），
无任何外部依赖。

安全提示：该服务器不含鉴权，默认仅绑定回环地址 127.0.0.1，用于本地调试与观测，
不应暴露到公网。

提供的 HTTP 端点：
  GET /                  返回前端单页应用（HTML）
  GET /api/info          返回 SDK 运行信息（JSON）
  GET /api/channels      返回全部渠道列表（JSON 数组）
  GET /api/logs/stream   以 SSE 持续推送 SDK 日志
"""
...
_log_handler: Optional["_WebUILogHandler"] = None
_prev_log_level: Optional[int] = None
_state_lock = threading.Lock()

# 当前 WebUI 绑定的主机地址（供 /api/info 回显）
_webui_host: str = "127.0.0.1"

# SSE 客户端队列集合，每个连接对应一个 queue.Queue
...
def _level_to_name(levelno: int) -> str:
    """将 logging 级别数值映射为前端使用的小写级别名"""
...
class _WebUILogHandler(logging.Handler):
    """自定义日志处理器：将 SDK 每条日志推送到 SSE 连接队列"""

    def emit(self, record: logging.LogRecord) -> None:
        try:
            entry = {
                "time": time.strftime("%H:%M:%S", time.localtime(record.created)),
                "level": _level_to_name(record.levelno),
...
def _render_html() -> str:
    """组装完整 HTML（首次调用后缓存）：将脚本注入模板占位符"""
    global _rendered_html
    if _rendered_html is None:
        _rendered_html = _INDEX_TEMPLATE.replace(
            "<!-- PLACEHOLDER_SCRIPT -->", _APP_SCRIPT
        )
    return _rendered_html


class _WebUIRequestHandler(BaseHTTPRequestHandler):
    """WebUI HTTP 请求处理器：路由静态页面、JSON API 与 SSE 日志流"""

    # 关闭标准库默认的 stderr 访问日志，避免污染 SDK 输出
    ...
    def _handle_info(self) -> None:
        """GET /api/info：返回 SDK 运行信息"""
        cfg = get_config()
        port = self.server.server_address[1] if self.server else 0
        host = self.server.server_address[0] if self.server else _webui_host
    ...
    def _handle_channels(self) -> None:
        """GET /api/channels：返回全部渠道列表"""
        data = [
            {"channel": c.channel, "name": c.name, "website": c.website}
            for c in list_channels()
        ]
        self._send_json(data)

    def _handle_logs_stream(self) -> None:
        """GET /api/logs/stream：以 SSE 持续推送日志条目"""
...
def _resolve_port() -> int:
    """解析监听端口：TEMPMAIL_WEBUI_PORT 有效则用之，否则 0（系统随机分配）"""
    raw = (os.environ.get("TEMPMAIL_WEBUI_PORT") or "").strip()
    if raw.isdigit():
        p = int(raw)
        if 0 <= p <= 65535:
            return p
...
def _resolve_host() -> str:
    """解析监听主机：TEMPMAIL_WEBUI_HOST 有效则用之，否则 127.0.0.1"""
    return (os.environ.get("TEMPMAIL_WEBUI_HOST") or "127.0.0.1").strip()


def start_webui(port: Optional[int] = None, host: Optional[str] = None) -> Optional[int]:
    """
    启动 WebUI HTTP 服务器（幂等：已启动则直接返回现有端口）

    :param port: 监听端口；为 None 时读取 TEMPMAIL_WEBUI_PORT，仍无则由系统随机分配
    :param host: 监听主机；为 None 时读取 TEMPMAIL_WEBUI_HOST，仍无则默认 127.0.0.1
    :return: 实际监听端口；启动失败返回 None
    """
    global _server, _server_thread, _log_handler, _prev_log_level, _webui_host
    ...
    with _state_lock:
        ...
        _webui_host = bind_host
        logger = get_logger()
        try:
            server = ThreadingHTTPServer((bind_host, bind_port), _WebUIRequestHandler)
        except OSError as exc:
            logger.error(f"WebUI 启动失败: {exc}")
            return None
        server.daemon_threads = True

        # 挂接日志处理器，并临时下调级别以便捕获全部日志推送到前端
        ...
        logger.info(f"WebUI 已启动: http://{bind_host}:{actual_port}")
        return actual_port


def stop_webui() -> None:
    """停止 WebUI HTTP 服务器并还原日志配置（未启动时为空操作）"""
    global _server, _server_thread, _log_handler, _prev_log_level
    ...
    # shutdown 会阻塞直到 serve_forever 退出，放在锁外避免与请求线程死锁
    try:
        server.shutdown()
        server.server_close()
    except Exception:
...
def _auto_start_from_env() -> None:
    """模块加载时检查环境变量，若 TEMPMAIL_WEBUI 为真则自动启动"""
    flag = (os.environ.get("TEMPMAIL_WEBUI") or "").strip().lower()
    if flag in ("1", "true", "yes", "on"):
        start_webui(host=_resolve_host())


# ---------------------------------------------------------------------------
# 前端资源（内联为多行字符串常量，避免打包时丢失静态文件）
# ---------------------------------------------------------------------------
