 ...
 *
 * 创建邮箱:
 *   1. GET / 获取 PHPSESSID 会话 Cookie + 从 HTML 正则提取
 * csrfToken（csrfToken" value="xxx"）
 *   2. POST /api/generate_email.php（body {}，头 X-CSRF-Token，带 Cookie）
 *      →
 * {"success":true,"email_address":"xxx@...","expires_at":"...","error":null}
 * 获取邮件:
 *   POST /api/get_emails.php（body {"email_address":"xxx@..."}，头
 * X-CSRF-Token，带 Cookie） → {"success":true,"emails":[...],"error":null}
 *
...
static int tmfyi_b64_encode(const unsigned char *in, size_t inlen, char *out,
                            size_t outcap) {
  static const char T[] =
...
static char *tmfyi_build_token(const char *csrf, const char *cookie_hdr) {
  ...
  if (tmfyi_b64_encode((const unsigned char *)json, jl,
                       tok + strlen(TMFYI_TOK_PREFIX),
                       bcap - strlen(TMFYI_TOK_PREFIX)) != 0) {
    free(json);
    free(tok);
    return NULL;
  }
...
static int tmfyi_parse_token(const char *tok, char **csrf, char **cookie_hdr) {
  *csrf = NULL;
  *cookie_hdr = NULL;
  if (!tok || strncmp(tok, TMFYI_TOK_PREFIX, strlen(TMFYI_TOK_PREFIX)) != 0)
    return -1;
...
tm_email_info_t *tm_provider_tempmail_fyi_generate(void) {
  ...
  const char *hdr_home[] = {
      ...
      "Accept-Language: en-US,en;q=0.9,zh-CN;q=0.8,zh;q=0.7", NULL};
  tm_http_response_t *r1 =
      tm_http_request(TM_HTTP_GET, TMFYI_BASE "/", hdr_home, NULL, timeout);
  ...
  tm_http_response_t *r2 =
      tm_http_request(TM_HTTP_POST, TMFYI_BASE "/api/generate_email.php",
                      hdr_create, "{}", timeout);
  ...
  const char *email = cJSON_GetStringValue(
      cJSON_GetObjectItemCaseSensitive(root, "email_address"));
  if (!cJSON_IsBool(success) || !cJSON_IsTrue(success) || !email ||
      !strchr(email, '@')) {
    cJSON_Delete(root);
    free(csrf);
    free(ck);
  ...
  const char *exp = cJSON_GetStringValue(
...
tm_email_t *tm_provider_tempmail_fyi_get_emails(const char *token,
  ...
  *count = -1;
  ...
  char *csrf = NULL;
  char *ck = NULL;
  if (tmfyi_parse_token(token, &csrf, &ck) != 0)
    return NULL;
  ...
  tm_http_response_t *resp = tm_http_request(
      TM_HTTP_POST, TMFYI_BASE "/api/get_emails.php", hdr, reqBody, timeout);
  free(reqBody);
  free(csrf);
  free(ck);
