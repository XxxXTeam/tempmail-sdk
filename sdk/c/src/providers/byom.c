...
tm_email_t* tm_provider_byom_get_emails(const char *email, int *count) {
    *count = -1;
    ...
    tm_http_response_t *resp = tm_http_request(TM_HTTP_GET, url, byom_headers, NULL, 15);
    ...
    *count = n;
    ...
    for (int i = 0; i < n; i++) {
        ...
        const char *content = TM_JSON_STR(cJSON_GetObjectItemCaseSensitive(raw, "content"), "");
        const char *text = TM_JSON_STR(cJSON_GetObjectItemCaseSensitive(raw, "text"), "");
        const char *created = TM_JSON_STR(cJSON_GetObjectItemCaseSensitive(raw, "created_at"), "");
        if (content[0]) cJSON_AddStringToObject(raw, "html", content);
        if (text[0] && !cJSON_GetObjectItemCaseSensitive(raw, "text")) {
            /* text 字段已存在，normalize 会直接取 */
        }
        if (created[0]) cJSON_AddStringToObject(raw, "date", created);
