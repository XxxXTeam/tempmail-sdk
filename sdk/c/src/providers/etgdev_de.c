...
#define ETGDEV_DE_DOMAIN "etgdev.de"

static void etgdev_de_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_etgdev_de_generate(void) {
  char local[13];
  etgdev_de_random_local(local, 12);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_ETGDEV_DE;
  {
    size_t cap = strlen(local) + strlen("@" ETGDEV_DE_DOMAIN) + 1;
    info->email = (char *)malloc(cap);
    if (!info->email) {
      tm_free_email_info(info);
      return NULL;
    }
    snprintf(info->email, cap, "%s@%s", local, ETGDEV_DE_DOMAIN);
  }
  return info;
}
