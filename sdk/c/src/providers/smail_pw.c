/**
 * smail.pw：POST _root.data intent=generate，Cookie __session；GET 拉取 RSC
 * 响应并解析邮件
 */
...
static char *tm_smail_extract_inbox(const char *body) {
  ...
  for (const char *p = body; *p; p++) {
    ...
    memcpy(tmp, p + 1, n);
    tmp[n] = '\0';
    if (strstr(tmp, "@smail.pw"))
      return tm_strdup(tmp);
    p = q;
  }
  return NULL;
}
...
static const char *smail_try_parse_mail_v1_nosubj(const char *p,
                                                  const char *recipient,
                                                  char *id, char *to, char *fn,
                                                  char *fa, char *subj,
                                                  long long *tms) {
  (void)recipient;
  if (strncmp(p, "\"id\",", 5) != 0)
  ...
  if (!p || strncmp(p, ",\"to_address\",", 14) != 0)
    return NULL;
  p += 14;
  p = read_jstr(p, to, 512);
  if (!p || strncmp(p, ",\"from_name\",", 13) != 0)
    return NULL;
  p += 13;
  p = read_jstr(p, fn, 256);
  if (!p || strncmp(p, ",\"from_address\",", 16) != 0)
    return NULL;
  p += 16;
  p = read_jstr(p, fa, 256);
  if (!p || strncmp(p, ",\"subject\",\"time\",", 20) != 0)
    return NULL;
  p += 20;
  subj[0] = '\0';
  char *endptr = NULL;
...
static const char *smail_try_parse_mail_v1(const char *p, const char *recipient,
  ...
  p += 16;
  p = read_jstr(p, fa, 256);
  if (!p || strncmp(p, ",\"subject\",", 11) != 0)
    return NULL;
  p += 11;
  p = read_jstr(p, subj, 512);
  if (!p || strncmp(p, ",\"time\",", 8) != 0)
    return NULL;
  p += 8;
  char *endptr = NULL;
  *tms = strtoll(p, &endptr, 10);
...
static char *tm_smail_fetch_body(const char *token, const char *mail_id) {
  if (!token || !mail_id || !*mail_id || strncmp(mail_id, "__smail_", 8) == 0)
    return NULL;
  char url[512];
  ...
  for (int i = 0; sm_get_headers_base[i] && gi < 19; i++)
    gh[gi++] = sm_get_headers_base[i];
  gh[gi] = NULL;
  ...
  char *html =
      (body && cJSON_IsString(body)) ? tm_strdup(body->valuestring) : NULL;
  cJSON_Delete(root);
  return html;
}
...
tm_email_info_t *tm_provider_smail_pw_generate(void) {
  tm_http_response_t *resp = tm_http_request(
      TM_HTTP_POST, SM_ROOT, sm_post_headers, "intent=generate", 15);
...
tm_email_t *tm_provider_smail_pw_get_emails(const char *token,
  ...
  *count = -1;
  ...
  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_GET, SM_ROOT, gh, NULL, 15);
  if (!resp || resp->status < 200 || resp->status >= 300) {
    tm_http_response_free(resp);
    return NULL;
  }
  ...
  for (const char *p = body; *p && nr < 64;) {
    ...
    long long tms = 0;
    const char *next =
        smail_try_parse_mail_v1(hit, email, id, to, fn, fa, subj, &tms);
    if (!next)
      next = smail_try_parse_mail_v1_nosubj(hit, email, id, to, fn, fa, subj,
                                            &tms);
    if (!next)
      next = smail_try_parse_mail_v2(hit, email, id, to, fn, fa, subj, &tms);
    if (next) {
      char from_comb[512];
