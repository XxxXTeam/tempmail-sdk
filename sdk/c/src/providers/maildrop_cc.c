...
/*
 * 发送 GraphQL 请求并返回 data 下指定字段的 cJSON（调用者持有返回的根对象，需 cJSON_Delete）
 * query 通过 cJSON 序列化为 {"query": "..."}，避免转义问题
 */
static cJSON *mdcc_do_graphql(const char *query, int timeout) {
    ...
    tm_http_response_t *resp = tm_http_request(TM_HTTP_POST, MDCC_GRAPHQL_URL, mdcc_headers, payload, timeout);
    free(payload);
...
static char *mdcc_build_inbox_query(const char *mailbox) {
    ...
    if (!q) { free(quoted); return NULL; }
    snprintf(q, cap, "query { inbox(mailbox: %s) { id headerfrom subject date } }", quoted);
    free(quoted);
    return q;
}
...
static char *mdcc_build_message_query(const char *mailbox, const char *id) {
    ...
    if (!sm || !si) { cJSON_Delete(sm); cJSON_Delete(si); return NULL; }
    char *qm = cJSON_PrintUnformatted(sm);
    char *qi = cJSON_PrintUnformatted(si);
    cJSON_Delete(sm);
    cJSON_Delete(si);
    if (!qm || !qi) { free(qm); free(qi); return NULL; }
    size_t cap = strlen(qm) + strlen(qi) + 96;
    char *q = (char *)malloc(cap);
    if (!q) { free(qm); free(qi); return NULL; }
    snprintf(q, cap, "query { message(mailbox: %s, id: %s) { id headerfrom subject date html } }", qm, qi);
    free(qm);
    free(qi);
    return q;
}
...
tm_email_t *tm_provider_maildrop_cc_get_emails(const char *token, const char *email, int *count) {
    ...
    *count = -1;
    ...
    if (n > MDCC_MAX_MAILS) n = MDCC_MAX_MAILS;
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (msgRoot) {
            ...
            cJSON *message = mdata ? cJSON_GetObjectItemCaseSensitive(mdata, "message") : NULL;
        ...
        const char *from = cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(src, "headerfrom"));
        cJSON_AddStringToObject(raw, "from", from ? from : "");
        const char *subject = cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(src, "subject"));
        cJSON_AddStringToObject(raw, "subject", subject ? subject : "");
        const char *date = cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(src, "date"));
