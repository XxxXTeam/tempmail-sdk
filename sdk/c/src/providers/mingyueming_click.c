/**
 * mailmomy 域名变体 mingyueming.click
...
#define CHANNEL_MINGYUEMING_CLICK_DOMAIN "mingyueming.click"

static void mingyueming_click_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_mingyueming_click_generate(void) {
  char local[11];
  mingyueming_click_random_local(local, 10);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_MINGYUEMING_CLICK;
  {
    size_t cap =
        strlen(local) + strlen("@" CHANNEL_MINGYUEMING_CLICK_DOMAIN) + 1;
    ...
    snprintf(info->email, cap, "%s@%s", local,
             CHANNEL_MINGYUEMING_CLICK_DOMAIN);
  }
  /* token 即邮箱地址 */
