/**
 * Mailinator 官方姊妹域名 bobmail.info
...
#define BOBMAIL_INFO_DOMAIN "bobmail.info"

static void bobmail_info_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_bobmail_info_generate(void) {
  char local[13];
  bobmail_info_random_local(local, 12);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_BOBMAIL_INFO;
  {
    size_t cap = strlen(local) + strlen("@" BOBMAIL_INFO_DOMAIN) + 1;
    info->email = (char *)malloc(cap);
    if (!info->email) {
      tm_free_email_info(info);
      return NULL;
    }
    snprintf(info->email, cap, "%s@%s", local, BOBMAIL_INFO_DOMAIN);
  }
  return info;
}
