/**
 * Mailinator 姊妹域名 disposable.nogonad.nl
...
#define CHANNEL_DISPOSABLE_NOGONAD_NL_DOMAIN "disposable.nogonad.nl"

static void disposable_nogonad_nl_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_disposable_nogonad_nl_generate(void) {
  char local[13];
  disposable_nogonad_nl_random_local(local, 12);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_DISPOSABLE_NOGONAD_NL;
  {
    size_t cap =
        strlen(local) + strlen("@" CHANNEL_DISPOSABLE_NOGONAD_NL_DOMAIN) + 1;
    ...
    snprintf(info->email, cap, "%s@%s", local,
             CHANNEL_DISPOSABLE_NOGONAD_NL_DOMAIN);
  }
  return info;
}
