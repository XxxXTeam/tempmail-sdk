...
#define CHANNEL_NUXH62_SPACE_DOMAIN "nuxh62.space"

static void nuxh62_space_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_nuxh62_space_generate(void) {
  char local[11];
  nuxh62_space_random_local(local, 10);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_NUXH62_SPACE;
  {
    size_t cap = strlen(local) + strlen("@" CHANNEL_NUXH62_SPACE_DOMAIN) + 1;
    info->email = (char *)malloc(cap);
    if (!info->email) {
      tm_free_email_info(info);
      return NULL;
    }
    snprintf(info->email, cap, "%s@%s", local, CHANNEL_NUXH62_SPACE_DOMAIN);
  }
  /* token 即邮箱地址 */
