...
#define M8R_DAVIDFUHR_DE_DOMAIN "m8r.davidfuhr.de"

static void m8r_davidfuhr_de_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_m8r_davidfuhr_de_generate(void) {
  char local[13];
  m8r_davidfuhr_de_random_local(local, 12);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_M8R_DAVIDFUHR_DE;
  {
    size_t cap = strlen(local) + strlen("@" M8R_DAVIDFUHR_DE_DOMAIN) + 1;
    info->email = (char *)malloc(cap);
    if (!info->email) {
      tm_free_email_info(info);
      return NULL;
    }
    snprintf(info->email, cap, "%s@%s", local, M8R_DAVIDFUHR_DE_DOMAIN);
  }
  return info;
}
