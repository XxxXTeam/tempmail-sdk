/**
 * moakt.com：HTML 收件箱 + tm_session Cookie；详情 GET
 * /{locale}/email/{uuid}/html
 */
...
static int mok_cookie_has_tm_session(const char *hdr) {
  ...
  while (*p) {
    while (*p == ' ' || *p == '\t')
      p++;
    if (tm_strncasecmp(p, name, (int)nl) == 0 && p[nl] == '=')
      return 1;
    const char *semi = strchr(p, ';');
    if (!semi)
      break;
...
static void mok_request_parts(const char *domain, char *locale,
                              size_t locale_cap, char *mail_domain,
                              size_t domain_cap) {
  if (locale && locale_cap > 0)
    locale[0] = '\0';
  if (mail_domain && domain_cap > 0)
    mail_domain[0] = '\0';
  if (!locale || locale_cap < 4 || !mail_domain || domain_cap < 4)
    return;
  if (!domain || !*domain) {
    strcpy(locale, "zh");
    return;
  ...
  if (mok_is_mail_domain(tmp)) {
    strcpy(locale, "zh");
    if (strlen(tmp) < domain_cap)
      strcpy(mail_domain, tmp);
    return;
  }
  if (len >= locale_cap) {
    strcpy(locale, "zh");
...
static int mok_b64_encode(const unsigned char *in, size_t inlen, char *out,
                          size_t outcap) {
  static const char T[] =
...
static char *mok_build_token(const char *locale, const char *cookie_hdr) {
  ...
  if (mok_b64_encode((const unsigned char *)json, jl,
                     tok + strlen(MOK_TOK_PREFIX),
                     bcap - strlen(MOK_TOK_PREFIX)) != 0) {
    free(json);
    free(tok);
    return NULL;
  }
...
static int mok_parse_token(const char *tok, char **locale, char **cookie_hdr) {
  *locale = NULL;
  *cookie_hdr = NULL;
  if (!tok || strncmp(tok, MOK_TOK_PREFIX, strlen(MOK_TOK_PREFIX)) != 0)
    return -1;
  ...
  if (!cJSON_IsString(jl) || !jl->valuestring || !cJSON_IsString(jc) ||
      !jc->valuestring) {
    cJSON_Delete(j);
    return -1;
  }
...
static int mok_parse_inbox_email(const char *html, char *out, size_t cap) {
  ...
  if (!id)
    id = strstr(html, "id='email-address'");
  if (!id)
    return -1;
  ...
  *b = '\0';
  if (a != out)
    memmove(out, a, (size_t)(b - a) + 1);
  return 0;
}

static int mok_is_uuid(const char *s) {
  if (!s || strlen(s) != MOK_UUID_LEN)
    return 0;
...
static void mok_collect_ids(const char *html, char ids[][40], int *nids,
                            int maxids) {
  *nids = 0;
  ...
  while (*p && *nids < maxids) {
    ...
    uid[MOK_UUID_LEN] = '\0';
    if (!mok_is_uuid(uid) ||
        (start[MOK_UUID_LEN] != '"' && start[MOK_UUID_LEN] != '\'')) {
      p = h;
      continue;
    }
    int dup = 0;
...
static char *mok_extract_tag_content(const char *html, const char *class_attr,
                                     const char *end_tag) {
  char pat[96];
...
static cJSON *mok_parse_detail_json(const char *page, const char *id,
  ...
  if (title) {
    ...
    cJSON_AddStringToObject(raw, "subject", "");

  char *dateblk = NULL;
  const char *dp = strstr(page, "class=\"date\"");
  if (!dp)
    dp = strstr(page, "class='date'");
  if (dp) {
    dp = strstr(dp, "<span");
    if (dp) {
      dp = strchr(dp, '>');
  ...
  if (!eb)
    eb = strstr(page, "class='email-body'");
  if (eb) {
    eb = strchr(eb, '>');
    if (eb) {
      eb++;
...
tm_email_info_t *tm_provider_moakt_generate(const char *domain) {
  ...
  tm_http_response_t *r1 =
      tm_http_request(TM_HTTP_GET, base, hdr_home, NULL, timeout);
  if (!r1 || r1->status != 200) {
    tm_http_response_free(r1);
    return NULL;
  }
  if (mail_domain[0] &&
      (!r1->body || !mok_page_has_domain(r1->body, mail_domain))) {
    tm_http_response_free(r1);
    return NULL;
  }
  ...
  if (mail_domain[0]) {
    ...
    snprintf(post_body, sizeof(post_body),
             "setemail=&username=%s&domain=%s&preferred_domain=", local,
             mail_domain);
  } else {
  ...
  tm_http_response_t *r2 =
      tm_http_request(TM_HTTP_POST, inbox, hdr_post, post_body, timeout);
  if (!r2) {
    free(ck);
    return NULL;
  }
  ...
  tm_http_response_t *r3 =
      tm_http_request(TM_HTTP_GET, inbox, hdr_inbox, NULL, timeout);
...
tm_email_t *tm_provider_moakt_get_emails(const char *token, const char *email,
  ...
  *count = -1;
  ...
  char *loc_heap = NULL;
  char *ck = NULL;
  if (mok_parse_token(token, &loc_heap, &ck) != 0)
    return NULL;
  ...
  for (int i = 0; i < nids; i++) {
    ...
    snprintf(url, sizeof(url), "%s/%s/email/%s/html", MOK_ORIGIN, loc_buf,
             ids[i]);
    snprintf(refer, sizeof(refer), "%s/%s/email/%s", MOK_ORIGIN, loc_buf,
             ids[i]);
    ...
    tm_http_response_t *rd =
        tm_http_request(TM_HTTP_GET, url, hdr_d, NULL, timeout);
    cJSON *raw = NULL;
