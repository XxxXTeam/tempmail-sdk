...
#define CHANNEL_B_SMELLY_CC_DOMAIN "b.smelly.cc"

static void b_smelly_cc_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_b_smelly_cc_generate(void) {
  char local[13];
  b_smelly_cc_random_local(local, 12);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_B_SMELLY_CC;
  {
    size_t cap = strlen(local) + strlen("@" CHANNEL_B_SMELLY_CC_DOMAIN) + 1;
    info->email = (char *)malloc(cap);
    if (!info->email) {
      tm_free_email_info(info);
      return NULL;
    }
    snprintf(info->email, cap, "%s@%s", local, CHANNEL_B_SMELLY_CC_DOMAIN);
  }
  return info;
}
