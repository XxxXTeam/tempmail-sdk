...
tm_email_info_t *tm_provider_guerrillamail_generate(void) {
  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_GET, GM_BASE "?f=get_email_address&lang=en",
                      gm_headers, NULL, 15);
...
tm_email_t *tm_provider_guerrillamail_get_emails(const char *token,
  ...
  *count = -1;
  char url[512];
  snprintf(url, sizeof(url), GM_BASE "?f=check_email&seq=0&sid_token=%s",
           token);

  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_GET, url, gm_headers, NULL, 15);
  if (!resp || resp->status != 200) {
    tm_http_response_free(resp);
    return NULL;
  }
  ...
  *count = n;
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (!body_str)
      body_str = cJSON_GetStringValue(
          cJSON_GetObjectItemCaseSensitive(item, "mail_excerpt"));
