...
#define MAIL_FSMASH_ORG_DOMAIN "mail.fsmash.org"

static void mail_fsmash_org_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_mail_fsmash_org_generate(void) {
  char local[13];
  mail_fsmash_org_random_local(local, 12);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_MAIL_FSMASH_ORG;
  {
    size_t cap = strlen(local) + strlen("@" MAIL_FSMASH_ORG_DOMAIN) + 1;
    info->email = (char *)malloc(cap);
    if (!info->email) {
      tm_free_email_info(info);
      return NULL;
    }
    snprintf(info->email, cap, "%s@%s", local, MAIL_FSMASH_ORG_DOMAIN);
  }
  return info;
}
