...
tm_email_info_t* tm_provider_anonymmail_generate(void) {
    ...
    /* 先获取域名列表 */
    tm_http_response_t *dom_resp = tm_http_request(TM_HTTP_POST, AM_BASE "/api/getDomains", am_headers, "", 15);
    ...
    const char *domain = TM_JSON_STR(cJSON_GetObjectItemCaseSensitive(pick, "domain"), "");
    if (!domain[0]) {
        cJSON_Delete(dom_root);
        return NULL;
    }
    ...
    tm_http_response_t *resp = tm_http_request(TM_HTTP_POST, AM_BASE "/api/create", am_headers, body, 15);
    ...
    const char *addr = cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(root, "email"));
    if (!addr || !addr[0]) addr = email_addr;
...
tm_email_t* tm_provider_anonymmail_get_emails(const char *email, int *count) {
    *count = -1;
    ...
    tm_http_response_t *resp = tm_http_request(TM_HTTP_POST, AM_BASE "/api/get", am_headers, body, 15);
    if (!resp || resp->status < 200 || resp->status >= 300) {
        tm_http_response_free(resp);
        return NULL;
    }
    ...
    *count = n;
    ...
    for (int i = 0; i < n; i++) {
        ...
        const char *token_val = TM_JSON_STR(cJSON_GetObjectItemCaseSensitive(raw, "token"), "");
        const char *body_val = TM_JSON_STR(cJSON_GetObjectItemCaseSensitive(raw, "body"), "");
        if (token_val[0]) cJSON_AddStringToObject(raw, "id", token_val);
        if (body_val[0]) cJSON_AddStringToObject(raw, "html", body_val);
