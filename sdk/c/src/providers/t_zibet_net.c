...
#define T_ZIBET_NET_DOMAIN "t.zibet.net"

static void t_zibet_net_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_t_zibet_net_generate(void) {
  char local[13];
  t_zibet_net_random_local(local, 12);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_T_ZIBET_NET;
  {
    size_t cap = strlen(local) + strlen("@" T_ZIBET_NET_DOMAIN) + 1;
    info->email = (char *)malloc(cap);
    if (!info->email) {
      tm_free_email_info(info);
      return NULL;
    }
    snprintf(info->email, cap, "%s@%s", local, T_ZIBET_NET_DOMAIN);
  }
  return info;
}
