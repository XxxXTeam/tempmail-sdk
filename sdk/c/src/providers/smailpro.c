 ...
 *
 * 两段式调用：
 *   1. GET https://smailpro.com/app/payload?url={目标API}[&email=&mid=] → 返回
 * JWT（纯文本）
 ...
 * target_url 为目标 sonjj 接口地址（未编码），email/mid 为附加查询参数（可为
 * NULL） 返回堆分配的 JWT 字符串，调用者负责 free；失败返回 NULL
 */
static char *smailpro_fetch_payload(const char *target_url, const char *email,
                                    const char *mid, int timeout) {
  ...
  if (enc_email)
    cap += strlen(enc_email) + 8;
  if (enc_mid)
    cap += strlen(enc_mid) + 6;
  ...
  if (enc_email)
    off += snprintf(url + off, cap - (size_t)off, "&email=%s", enc_email);
  if (enc_mid)
    off += snprintf(url + off, cap - (size_t)off, "&mid=%s", enc_mid);
  free(enc_url);
  free(enc_email);
  free(enc_mid);

  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_GET, url, smailpro_headers, NULL, timeout);
  free(url);
 ...
 */
static cJSON *smailpro_call_api(const char *target_url, const char *email,
                                const char *mid, int timeout) {
...
tm_email_info_t *tm_provider_smailpro_generate(void) {
  ...
  cJSON *root =
      smailpro_call_api(SMAILPRO_API_BASE "/create", NULL, NULL, timeout);
  if (!root)
    return NULL;
...
tm_email_t *tm_provider_smailpro_get_emails(const char *token,
  ...
  *count = -1;
  ...
  cJSON *root =
      smailpro_call_api(SMAILPRO_API_BASE "/inbox", email, NULL, timeout);
  if (!root)
    return NULL;
  ...
  if (n > SMAILPRO_MAX_MAILS)
    n = SMAILPRO_MAX_MAILS;
  ...
  for (int i = 0; i < n; i++) {
    ...
    const char *datetime = cJSON_GetStringValue(
        cJSON_GetObjectItemCaseSensitive(item, "datetime"));
    cJSON_AddStringToObject(raw, "date", datetime ? datetime : "");

    /* 拉取正文（body=HTML, textBody=纯文本），失败时保留列表元信息 */
    if (mid && mid[0]) {
      cJSON *msg =
          smailpro_call_api(SMAILPRO_API_BASE "/message", email, mid, timeout);
      if (msg) {
        const char *body =
            cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(msg, "body"));
        const char *textBody = cJSON_GetStringValue(
            cJSON_GetObjectItemCaseSensitive(msg, "textBody"));
        if (body && body[0])
          cJSON_AddStringToObject(raw, "html", body);
        if (textBody && textBody[0])
          cJSON_AddStringToObject(raw, "text", textBody);
        cJSON_Delete(msg);
      }
    }
