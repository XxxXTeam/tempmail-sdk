...
static const char *tmio_cors_header(void) {
  ...
  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_GET, TMIO_PAGE, headers, NULL, 15);
  if (resp && resp->body) {
...
static void tmio_headers(const char **headers, char *cors_line,
                         size_t cors_line_cap) {
  ...
  headers[3] = cors_line;
  headers[4] = "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
...
tm_email_info_t *tm_provider_temp_mail_io_generate(void) {
  ...
  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_POST, TMIO_BASE "/email/new", headers, body, 15);
...
tm_email_t *tm_provider_temp_mail_io_get_emails(const char *email, int *count) {
  *count = -1;
  ...
  size_t url_len =
      strlen(TMIO_BASE) + strlen("/email//messages") + strlen(email) + 1;
