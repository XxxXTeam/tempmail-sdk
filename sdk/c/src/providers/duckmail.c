...
static const char* duckmail_headers[] = {
...
tm_email_info_t* tm_provider_duckmail_generate(void) {
    ...
    tm_http_response_t *resp = tm_http_request(TM_HTTP_GET, DUCKMAIL_BASE "/domains?page=1", duckmail_headers, NULL, 15);
    ...
    resp = tm_http_request(TM_HTTP_POST, DUCKMAIL_BASE "/accounts", create_headers, body, 15);
    if (!resp || resp->status < 200 || resp->status >= 300) { tm_http_response_free(resp); return NULL; }
    tm_http_response_free(resp);

    resp = tm_http_request(TM_HTTP_POST, DUCKMAIL_BASE "/token", duckmail_headers, body, 15);
    if (!resp || resp->status != 200) { tm_http_response_free(resp); return NULL; }
...
tm_email_t* tm_provider_duckmail_get_emails(const char *token, const char *email, int *count) {
    *count = -1;
    ...
    tm_http_response_t *resp = tm_http_request(TM_HTTP_GET, DUCKMAIL_BASE "/messages?page=1", headers, NULL, 15);
    if (!resp || resp->status != 200) { tm_http_response_free(resp); return NULL; }
    ...
    *count = n;
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (mid) {
            char detail_url[512];
            snprintf(detail_url, sizeof(detail_url), DUCKMAIL_BASE "/messages/%s", mid);
