...
static char *cg_gm_sid_value(const char *cookie_line) {
  if (!cookie_line || strncmp(cookie_line, "gm_sid=", 7) != 0)
    return NULL;
  return tm_strdup(cookie_line + 7);
}

static int cg_retry_home(long status, const char *cookie_line,
                         const char *browser) {
  if (status == 401)
    return 1;
  if (!cookie_line || !browser)
    return 1;
  return 0;
}

static int cg_fetch_home_session(char **cookie_line_out, char **browser_out,
                                 long *status_out) {
  if (status_out)
    *status_out = 0;
  ...
  *browser_out = NULL;
  ...
  tm_http_response_t *home =
      tm_http_request(TM_HTTP_GET, CG_HOME, cg_headers, NULL, 15);
  if (!home)
    return -1;
  if (status_out)
    *status_out = home->status;
  if (home->status != 200) {
...
static int cg_fetch_home_session_with_retry(char **cookie_line_out,
                                            char **browser_out) {
  long st = 0;
...
static char *cg_fetch_inbox_token_once(const char *email,
...
static int cg_refresh_inbox_pair(const char *email, char **inbox_out,
                                 char **gm_val_out) {
  char *cookie_line = NULL;
  char *browser = NULL;
  if (cg_fetch_home_session_with_retry(&cookie_line, &browser) != 0) {
...
static int cg_parse_packed(const char *token, char **gm_val_out,
                           char **inbox_out) {
  *gm_val_out = NULL;
  *inbox_out = NULL;
  if (!token || !token[0])
...
static tm_email_t *cg_fetch_emails_once(const char *inbox_token,
                                        const char *email, const char *gm_val,
                                        int *count, long *status_out) {
  if (count)
    *count = -1;
  if (status_out)
    *status_out = 0;
  if (!inbox_token || !inbox_token[0] || !gm_val || !gm_val[0])
    return NULL;
...
tm_email_info_t *tm_provider_chatgpt_org_uk_generate(void) {
  ...
  tm_http_response_t *resp = tm_http_request(
      TM_HTTP_GET, CG_BASE "/generate-email", gen_headers, NULL, 15);
  ...
  cJSON *data = cJSON_GetObjectItemCaseSensitive(json, "data");
  const char *email =
      cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(data, "email"));
  if (!email || !email[0]) {
    cJSON_Delete(json);
    free(cookie_line);
    return NULL;
...
tm_email_t *tm_provider_chatgpt_org_uk_get_emails(const char *token,
  ...
  *count = -1;
  ...
  long status = 0;
  tm_email_t *emails =
      cg_fetch_emails_once(inbox, email, gm_val, count, &status);
