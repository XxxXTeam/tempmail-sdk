...
#define M_NIK_ME_DOMAIN "m.nik.me"

static void m_nik_me_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_m_nik_me_generate(void) {
  char local[13];
  m_nik_me_random_local(local, 12);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_M_NIK_ME;
  {
    size_t cap = strlen(local) + strlen("@" M_NIK_ME_DOMAIN) + 1;
    info->email = (char *)malloc(cap);
    if (!info->email) {
      tm_free_email_info(info);
      return NULL;
    }
    snprintf(info->email, cap, "%s@%s", local, M_NIK_ME_DOMAIN);
  }
  return info;
}
