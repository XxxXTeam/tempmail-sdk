...
static char *gm_mirror_fetch_body(const char *base_url, const char *token,
                                  const cJSON *mid) {
  if (!mid)
    return NULL;
  char id_buf[64];
  ...
  if (!id || !id[0])
    return NULL;
  char url[768];
  snprintf(url, sizeof(url), "%s?f=fetch_email&sid_token=%s&email_id=%s",
           base_url, token, id);
  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_GET, url, gm_mirror_headers, NULL, 15);
  ...
  if (!json)
    return NULL;
  const char *body =
      cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(json, "mail_body"));
...
tm_provider_guerrillamail_mirror_generate(tm_channel_t channel,
  ...
  const char *addr = cJSON_GetStringValue(
      cJSON_GetObjectItemCaseSensitive(json, "email_addr"));
  const char *sid =
      cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(json, "sid_token"));
  if (!addr || !sid) {
    cJSON_Delete(json);
    return NULL;
  }
...
tm_email_t *tm_provider_guerrillamail_mirror_get_emails(const char *base_url,
  ...
  *count = -1;
  char url[512];
  snprintf(url, sizeof(url), "%s?f=check_email&seq=0&sid_token=%s", base_url,
           token);
  ...
  *count = n;
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (mid)
      cJSON_AddNumberToObject(flat, "id", mid->valuedouble);
    cJSON_AddStringToObject(
        flat, "from",
        TM_JSON_STR(cJSON_GetObjectItemCaseSensitive(item, "mail_from"), ""));
    cJSON_AddStringToObject(flat, "to", email);
    cJSON_AddStringToObject(
        flat, "subject",
        TM_JSON_STR(cJSON_GetObjectItemCaseSensitive(item, "mail_subject"),
                    ""));
    char *body_owned = NULL;
    const char *body_str = cJSON_GetStringValue(
        cJSON_GetObjectItemCaseSensitive(item, "mail_body"));
    ...
    if (body_str && body_str[0]) {
      ...
      cJSON_AddStringToObject(
          flat, "text",
          TM_JSON_STR(cJSON_GetObjectItemCaseSensitive(item, "mail_excerpt"),
                      ""));
    }
    free(body_owned);
    cJSON_AddStringToObject(
        flat, "date",
        TM_JSON_STR(cJSON_GetObjectItemCaseSensitive(item, "mail_date"), ""));
    cJSON *mr = cJSON_GetObjectItemCaseSensitive(item, "mail_read");
    if (mr)
      cJSON_AddBoolToObject(flat, "isRead", mr->valueint == 1);
