...
#define CHANNEL_MINGYUKEJI_LOL_DOMAIN "mingyukeji.lol"

static void mingyukeji_lol_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_mingyukeji_lol_generate(void) {
  char local[11];
  mingyukeji_lol_random_local(local, 10);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_MINGYUKEJI_LOL;
  {
    size_t cap = strlen(local) + strlen("@" CHANNEL_MINGYUKEJI_LOL_DOMAIN) + 1;
    info->email = (char *)malloc(cap);
    if (!info->email) {
      tm_free_email_info(info);
      return NULL;
    }
    snprintf(info->email, cap, "%s@%s", local, CHANNEL_MINGYUKEJI_LOL_DOMAIN);
  }
  /* token 即邮箱地址 */
