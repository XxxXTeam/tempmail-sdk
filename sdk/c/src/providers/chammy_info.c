...
#define CHAMMY_INFO_DOMAIN "chammy.info"

static void chammy_info_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_chammy_info_generate(void) {
  char local[13];
  chammy_info_random_local(local, 12);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_CHAMMY_INFO;
  {
    size_t cap = strlen(local) + strlen("@" CHAMMY_INFO_DOMAIN) + 1;
    info->email = (char *)malloc(cap);
    if (!info->email) {
      tm_free_email_info(info);
      return NULL;
    }
    snprintf(info->email, cap, "%s@%s", local, CHAMMY_INFO_DOMAIN);
  }
  return info;
}
