...
/* 备用域名（API 不可用时使用） */
static const char *tm365_fallback_domains[] = {"fengyou.cc", "shop345.com",
                                               "nutemail.com", "qvrf.cn"};
...
tm_email_info_t *tm_tempmail365_generate(const char *domain) {
  ...
  tm_http_response_t *resp = tm_http_request(
      TM_HTTP_GET, TM365_BASE "?action=get_config", tm365_headers, NULL, 15);
  ...
  char url[1024];
  snprintf(url, sizeof(url), "%s?action=create_email&email=%s&domain=%s",
           TM365_BASE, enc_email, enc_domain);
  free(enc_email);
  free(enc_domain);

  tm_http_response_t *r2 =
      tm_http_request(TM_HTTP_GET, url, tm365_headers, NULL, 15);
...
tm_email_t *tm_tempmail365_get_emails(const char *email, int *count) {
  *count = -1;
  ...
  char url[1024];
  snprintf(url, sizeof(url), "%s?action=fetch_mail&email=%s", TM365_BASE,
           enc_email);
  free(enc_email);
  ...
  const char *content =
      cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(root, "content"));
