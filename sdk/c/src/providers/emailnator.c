 ...
 */
static int en_parse_cookies(const char *raw_cookies, char **xsrf_out,
                            char **cookies_out) {
  *xsrf_out = NULL;
  *cookies_out = NULL;
  if (!raw_cookies)
  ...
  if (!xsrf_end)
    xsrf_end = xsrf_start + strlen(xsrf_start);
...
tm_email_info_t *tm_provider_emailnator_generate(void) {
  const char *init_headers[] = {"User-Agent: Mozilla/5.0 (Windows NT 10.0; "
                                "Win64; x64) AppleWebKit/537.36",
                                NULL};

  /* 1. 初始化 session，获取 cookie */
  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_GET, EN_BASE, init_headers, NULL, 15);
  ...
  const char *gen_headers[] = {"User-Agent: Mozilla/5.0 (Windows NT 10.0; "
                               "Win64; x64) AppleWebKit/537.36",
  ...
  resp = tm_http_request(
      TM_HTTP_POST, EN_BASE "/generate-email", gen_headers,
      "{\"email\":[\"plusGmail\",\"dotGmail\",\"googleMail\"]}", 15);
  if (!resp || resp->status != 200) {
    free(xsrf_token);
    free(cookies);
    tm_http_response_free(resp);
...
static char *en_fetch_detail(const char *xsrf, const char *cookies,
                             const char *email, const char *message_id) {
  if (!message_id || !message_id[0])
    return tm_strdup("");
  char xsrf_header[1024], cookie_header[2048], body[1024];
  snprintf(xsrf_header, sizeof(xsrf_header), "X-XSRF-TOKEN: %s", xsrf);
  snprintf(cookie_header, sizeof(cookie_header), "Cookie: %s", cookies);
  snprintf(body, sizeof(body), "{\"email\":\"%s\",\"messageID\":\"%s\"}", email,
           message_id);

  const char *headers[] = {"User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; "
                           ...
                           NULL};
  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_POST, EN_BASE "/message-list", headers, body, 15);
  if (!resp || resp->status != 200) {
    tm_http_response_free(resp);
    return tm_strdup("");
  }
...
tm_email_t *tm_provider_emailnator_get_emails(const char *token,
  ...
  *count = -1;
  ...
  const char *xsrf = cJSON_GetStringValue(
      cJSON_GetObjectItemCaseSensitive(session, "xsrfToken"));
  const char *cookies = cJSON_GetStringValue(
      cJSON_GetObjectItemCaseSensitive(session, "cookies"));
  if (!xsrf || !cookies) {
  ...
  for (int i = 0; i < total; i++) {
    ...
    const char *mid = cJSON_GetStringValue(
        cJSON_GetObjectItemCaseSensitive(msg, "messageID"));
    if (mid && strncmp(mid, "ADS", 3) != 0)
      real_count++;
  }
  ...
  *count = real_count;
  ...
  for (int i = 0; i < total && idx < real_count; i++) {
    ...
    cJSON_AddStringToObject(
        flat, "date",
        TM_JSON_STR(cJSON_GetObjectItemCaseSensitive(msg, "time"), ""));
