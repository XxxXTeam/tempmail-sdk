/**
 * mailmomy 域名变体 mingyuekeji.online
...
#define CHANNEL_MINGYUEKEJI_ONLINE_DOMAIN "mingyuekeji.online"

static void mingyuekeji_online_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_mingyuekeji_online_generate(void) {
  char local[11];
  mingyuekeji_online_random_local(local, 10);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_MINGYUEKEJI_ONLINE;
  {
    size_t cap =
        strlen(local) + strlen("@" CHANNEL_MINGYUEKEJI_ONLINE_DOMAIN) + 1;
    ...
    snprintf(info->email, cap, "%s@%s", local,
             CHANNEL_MINGYUEKEJI_ONLINE_DOMAIN);
  }
  /* token 即邮箱地址 */
