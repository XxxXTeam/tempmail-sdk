 ...
 *   返回: JSON {id, subject, sender, sender_email, received_at, body_html,
 * advertisement}
 *
 * token 存储策略:
 *   存储 username（@前面的部分）
...
tm_email_info_t *tm_provider_mailtemp_cc_generate(void) {
  ...
  tm_http_response_t *resp = tm_http_request(TM_HTTP_POST, MTC_API, mtc_headers,
                                             "action=inbox", timeout);

  if (!resp || resp->status != 200 || !resp->body) {
    TM_LOG_ERR("[mailtemp-cc] 创建收件箱请求失败, status=%ld",
               resp ? resp->status : 0);
 ...
 *   2. 对每封邮件，POST api.php, body: action=view&id={id}&inbox={username}
 * 获取详情
 *   3. 将详情标准化后返回
 *
...
tm_email_t *tm_provider_mailtemp_cc_get_emails(const char *token,
  ...
  *count = -1;
  ...
  size_t fetch_body_len =
      strlen("action=fetch&inbox=") + strlen(token) + strlen("&last_id=0") + 1;
  char *fetch_body = (char *)malloc(fetch_body_len);
  if (!fetch_body)
    return NULL;
  snprintf(fetch_body, fetch_body_len, "action=fetch&inbox=%s&last_id=0",
           token);

  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_POST, MTC_API, mtc_headers, fetch_body, timeout);
  free(fetch_body);

  if (!resp || resp->status != 200 || !resp->body) {
    TM_LOG_ERR("[mailtemp-cc] 获取邮件列表请求失败, status=%ld",
               resp ? resp->status : 0);
    tm_http_response_free(resp);
    return NULL;
  }
  ...
  if (list_size > MTC_MAX_MAILS)
    list_size = MTC_MAX_MAILS;
  ...
  for (int i = 0; i < list_size; i++) {
    ...
    size_t view_body_len = strlen("action=view&id=") + strlen(id_str) +
                           strlen("&inbox=") + strlen(token) + 1;
    char *view_body = (char *)malloc(view_body_len);
    if (!view_body)
      continue;
    snprintf(view_body, view_body_len, "action=view&id=%s&inbox=%s", id_str,
             token);

    tm_http_response_t *detail_resp =
        tm_http_request(TM_HTTP_POST, MTC_API, mtc_headers, view_body, timeout);
    free(view_body);
    ...
    if (!detail_resp || detail_resp->status != 200 || !detail_resp->body) {
      ...
      const cJSON *j_sender_email =
          cJSON_GetObjectItemCaseSensitive(raw, "sender_email");
    ...
    const cJSON *j_sender_email =
        cJSON_GetObjectItemCaseSensitive(detail, "sender_email");
    if (cJSON_IsString(j_sender_email) && j_sender_email->valuestring) {
    ...
    /* 映射 body_html -> html */
    const cJSON *j_body_html =
        cJSON_GetObjectItemCaseSensitive(detail, "body_html");
    ...
    /* 映射 received_at -> date */
    const cJSON *j_received =
        cJSON_GetObjectItemCaseSensitive(detail, "received_at");
    if (cJSON_IsString(j_received) && j_received->valuestring) {
