...
static const char *ab_headers_html[] = {
    ...
    "Accept: text/html,application/xhtml+xml", NULL};
...
static int parse_mail_link(const char *html, char *inbox, size_t inbox_cap,
...
static int find_address_local(const char *html, char *local_out,
                              size_t local_cap) {
  const char *region_start = strstr(html, "Your one-day-email-address is:");
  if (!region_start)
    return -1;
  const char *region_end =
      strstr(region_start, "You can check your mail here:");
  if (!region_end)
    region_end = html + strlen(html);

  for (const char *p = region_start;
       p < region_end && (p = strstr(p, "<dd")) != NULL;) {
    ...
    while (loc_len > 0 && isspace((unsigned char)stripped[loc_len - 1]))
      loc_len--;
    if (loc_len == 0 || loc_len >= local_cap)
      return -1;
...
static void qp_decode(const char *body, const char *hdrs, char *out,
                      size_t cap) {
  const char *enc = strstr(hdrs, "Content-Transfer-Encoding");
  if (!enc)
    enc = strstr(hdrs, "content-transfer-encoding");
  int is_qp = 0;
  if (enc) {
    if (strstr(enc, "quoted-printable") != NULL ||
        strstr(enc, "Quoted-Printable") != NULL)
      is_qp = 1;
  }
  if (!is_qp) {
...
static cJSON *mbox_block_to_json(const char *block, const char *recipient) {
  ...
  char from[512] = "", to[512] = "", subj[512] = "", date[256] = "",
       ctype[512] = "";
    ...
    if (colon) {
      ...
      if (kl < sizeof(key)) {
        ...
        if (strcmp(key, "from") == 0)
          strncpy(from, vbuf, sizeof(from) - 1);
        else if (strcmp(key, "to") == 0)
          strncpy(to, vbuf, sizeof(to) - 1);
        else if (strcmp(key, "subject") == 0)
          strncpy(subj, vbuf, sizeof(subj) - 1);
        else if (strcmp(key, "date") == 0)
          strncpy(date, vbuf, sizeof(date) - 1);
        else if (strcmp(key, "content-type") == 0)
          strncpy(ctype, vbuf, sizeof(ctype) - 1);
      }
    }
    ...
    if (bnd) {
      ...
      if (bi > 0) {
        ...
        for (;;) {
          ...
          char ph_buf[4096];
          if (ph_len >= sizeof(ph_buf))
            ph_len = sizeof(ph_buf) - 1;
          ...
          if (strcmp(pct, "text/plain") == 0 && text[0] == '\0')
            strncpy(text, dec, sizeof(text) - 1);
          else if (strcmp(pct, "text/html") == 0 && html[0] == '\0')
            strncpy(html, dec, sizeof(html) - 1);
          part = pb;
        }
      }
    }
  ...
  if (text[0] == '\0' && html[0] == '\0') {
    ...
    if (strstr(low_hdr, "text/html") != NULL)
      strncpy(html, dec, sizeof(html) - 1);
    else
      strncpy(text, dec, sizeof(text) - 1);
  }
  ...
  if (to[0] == '\0')
    strncpy(to, recipient, sizeof(to) - 1);
  char idbuf[512];
  idbuf[0] = '\0';
...
tm_email_info_t *tm_provider_anonbox_generate(void) {
  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_GET, AB_PAGE, ab_headers_html, NULL, 15);
  if (!resp || resp->status != 200) {
    tm_http_response_free(resp);
    return NULL;
  }
  char inbox[64], secret[64], local[256];
  if (parse_mail_link(resp->body, inbox, sizeof(inbox), secret,
                      sizeof(secret)) != 0) {
    tm_http_response_free(resp);
    return NULL;
  }
...
tm_email_t *tm_provider_anonbox_get_emails(const char *token, const char *email,
  ...
  *count = -1;
  ...
  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_GET, url, ab_headers_plain, NULL, 15);
  if (!resp)
    return NULL;
  if (resp->status == 404) {
    tm_http_response_free(resp);
