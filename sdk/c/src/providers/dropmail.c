 ...
 * 配置：环境变量
 * DROPMAIL_AUTH_TOKEN、DROPMAIL_API_TOKEN、DROPMAIL_NO_AUTO_TOKEN、DROPMAIL_RENEW_LIFETIME
 */
...
static const char *dm_json_headers[] = {
    ...
    "Origin: https://dropmail.me", "Referer: https://dropmail.me/api/", NULL};
...
static time_t dm_deadline_for_lifetime(const char *life) {
  ...
  if (!life || !*life)
    return now + 50 * 60;
  if (strcasecmp(life, "1h") == 0)
    return now + 50 * 60;
  if (strcasecmp(life, "1d") == 0)
    return now + 23 * 3600;
  {
    ...
    if (n > 1 && (life[n - 1] == 'd' || life[n - 1] == 'D')) {
      int days = atoi(life);
      if (days > 1)
        return now + (time_t)(days - 1) * 24 * 3600;
    }
  }
  return now + 50 * 60;
}
...
static int dm_af_cache_fresh(void) {
  if (!dm_af_cached || dm_af_deadline == 0)
    return 0;
  time_t now = time(NULL);
...
static int dm_resolve_auto_af(char *tok_out, size_t cap) {
  ...
  if (dm_af_cached) {
    ...
    tm_http_response_t *resp =
        tm_http_request(TM_HTTP_POST, DM_REN_URL, dm_json_headers, jb, 15);
    free(jb);

    if (resp && resp->status == 200 && resp->body &&
        dm_parse_af_json(resp->body, tok_out, cap) == 0) {
      dm_af_cache_clear();
      dm_af_cached = tm_strdup(tok_out);
      if (dm_af_cached)
        dm_af_deadline = dm_deadline_for_lifetime(rlife);
...

  {
    ...
    tm_http_response_t *resp = tm_http_request(TM_HTTP_POST, DM_GEN_URL,
                                               dm_json_headers, gen_body, 15);
    if (!resp || resp->status != 200 || !resp->body ||
        dm_parse_af_json(resp->body, tok_out, cap) != 0) {
      tm_http_response_free(resp);
      return -1;
    ...
    dm_af_cached = tm_strdup(tok_out);
    if (dm_af_cached)
      dm_af_deadline = time(NULL) + 50 * 60;
    return 0;
  }
}
...
static int dm_graphql_url(char *url, size_t cap) {
  ...
  if (ex) {
    ...
    if (dm_resolve_auto_af(tok, sizeof(tok)) != 0)
      return -1;
  }
...
tm_email_info_t *tm_provider_dropmail_generate(void) {
  char gql[1024];
  if (dm_graphql_url(gql, sizeof(gql)) != 0)
    return NULL;

  const char *body = "query=mutation%20%7BintroduceSession%20%7Bid%2C%"
                     "20expiresAt%2C%20addresses%7Bid%2C%20address%7D%7D%7D";
  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_POST, gql, dm_form_headers, body, 15);
  ...
  info->email = tm_strdup(
      cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(first, "address")));
  info->token = tm_strdup(
      cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(session, "id")));
  cJSON_Delete(json);
  return info;
}
...
tm_email_t *tm_provider_dropmail_get_emails(const char *token,
  ...
  *count = -1;
  ...
  snprintf(body, sizeof(body),
           "query=query%%20(%%24id%%3A%%20ID!)%%20%%7Bsession(id%%3A%%24id)%%"
           "20%%7Bmails%%20%%7Bid%%2C%%20fromAddr%%2C%%20toAddr%%2C%%"
           "20receivedAt%%2C%%20text%%2C%%20headerSubject%%2C%%20html%%7D%%7D%%"
           "7D&variables=%%7B%%22id%%22%%3A%%22%s%%22%%7D",
           token);
  ...
  *count = n;
  ...
  for (int i = 0; i < n; i++) {
    ...
    cJSON_AddStringToObject(
        flat, "id", TM_JSON_STR(cJSON_GetObjectItemCaseSensitive(m, "id"), ""));
    cJSON_AddStringToObject(
        flat, "from",
        TM_JSON_STR(cJSON_GetObjectItemCaseSensitive(m, "fromAddr"), ""));
    cJSON_AddStringToObject(
        flat, "to",
        TM_JSON_STR(cJSON_GetObjectItemCaseSensitive(m, "toAddr"), email));
    cJSON_AddStringToObject(
        flat, "subject",
        TM_JSON_STR(cJSON_GetObjectItemCaseSensitive(m, "headerSubject"), ""));
    cJSON_AddStringToObject(
        flat, "text",
    ...
    cJSON_AddStringToObject(
        flat, "received_at",
        TM_JSON_STR(cJSON_GetObjectItemCaseSensitive(m, "receivedAt"), ""));
