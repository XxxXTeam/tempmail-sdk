...
static int mohmal_ck_parse_merge(mohmal_ck_t *tab, int n, int maxn, const char *hdr) {
    if (!hdr || !*hdr) return n;
    const char *p = hdr;
    while (*p) {
        while (*p == ' ' || *p == '\t' || *p == ';') p++;
...
static int mohmal_cookie_has_session(const char *hdr) {
    ...
    while (*p) {
        while (*p == ' ' || *p == '\t') p++;
        if (mohmal_strncasecmp(p, name, (int)nl) == 0 && p[nl] == '=') return 1;
        const char *semi = strchr(p, ';');
        if (!semi) break;
        p = semi + 1;
    }
...
static int mohmal_b64_encode(const unsigned char *in, size_t inlen, char *out, size_t outcap) {
...
static char *mohmal_build_token(const char *cookie_hdr) {
    ...
    if (mohmal_b64_encode((const unsigned char *)json, jl, tok + strlen(MOHMAL_TOK_PREFIX), bcap - strlen(MOHMAL_TOK_PREFIX)) != 0) {
        free(json);
        free(tok);
        return NULL;
    }
...
static int mohmal_parse_token(const char *tok, char **cookie_hdr) {
    *cookie_hdr = NULL;
    if (!tok || strncmp(tok, MOHMAL_TOK_PREFIX, strlen(MOHMAL_TOK_PREFIX)) != 0) return -1;
...
static void mohmal_collect_ids(const char *html, char ids[][64], int *nids, int maxids) {
    *nids = 0;
    ...
    while (*p && *nids < maxids) {
        ...
        for (int i = 0; i < *nids; i++) {
            if (strcmp(ids[i], uid) == 0) { dup = 1; break; }
        }
...
static int mohmal_parse_row_info(const char *html, const char *id, mohmal_row_info_t *info) {
    ...
    while (tr_start > html) {
        if (tr_start[0] == '<' &&
            (tr_start[1] == 't' || tr_start[1] == 'T') &&
            (tr_start[2] == 'r' || tr_start[2] == 'R')) break;
        tr_start--;
    }
    const char *tr_end = strstr(link, "</tr>");
    if (!tr_end) tr_end = html + strlen(html);
    ...
    while (p < tr_end && td_idx < 4) {
        ...
        if (!td_gt || td_gt >= tr_end) break;
        td_gt++;
        const char *td_close = strstr(td_gt, "</td>");
        if (!td_close || td_close > tr_end) td_close = tr_end;
        size_t cell_len = (size_t)(td_close - td_gt);
        char cell_buf[1024];
        if (cell_len >= sizeof(cell_buf)) cell_len = sizeof(cell_buf) - 1;
...
static char *mohmal_extract_message_body(const char *page) {
    ...
    if (!end) end = strstr(gt, "</section>");
    if (!end) end = page + strlen(page);
...
tm_email_info_t *tm_provider_mohmal_generate(void) {
    ...
    tm_http_response_t *r1 = tm_http_request(TM_HTTP_GET, create_url, hdr_create, NULL, timeout);
    ...
    if (!body || !strstr(body, "data-email")) {
        ...
        tm_http_response_t *r2 = tm_http_request(TM_HTTP_GET, MOHMAL_ORIGIN "/en/inbox", hdr_inbox, NULL, timeout);
...
tm_email_t *tm_provider_mohmal_get_emails(const char *token, const char *email, int *count) {
    *count = -1;
    ...
    tm_http_response_t *r = tm_http_request(TM_HTTP_GET, MOHMAL_ORIGIN "/en/inbox", hdr_inbox, NULL, timeout);
    ...
    if (nids > 0) {
        row_infos = (mohmal_row_info_t *)calloc((size_t)nids, sizeof(mohmal_row_info_t));
        if (row_infos) {
            for (int i = 0; i < nids; i++) {
    ...
    for (int i = 0; i < nids; i++) {
        ...
        tm_http_response_t *rd = tm_http_request(TM_HTTP_GET, url, hdr_detail, NULL, timeout);
