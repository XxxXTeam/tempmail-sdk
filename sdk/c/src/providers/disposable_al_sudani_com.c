...
#define CHANNEL_DISPOSABLE_AL_SUDANI_COM_DOMAIN "disposable.al-sudani.com"

static void disposable_al_sudani_com_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_disposable_al_sudani_com_generate(void) {
  char local[13];
  disposable_al_sudani_com_random_local(local, 12);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_DISPOSABLE_AL_SUDANI_COM;
  {
    size_t cap =
        strlen(local) + strlen("@" CHANNEL_DISPOSABLE_AL_SUDANI_COM_DOMAIN) + 1;
    ...
    snprintf(info->email, cap, "%s@%s", local,
             CHANNEL_DISPOSABLE_AL_SUDANI_COM_DOMAIN);
  }
  return info;
}
