...
static cJSON *mailinator_request_json(const char *url) {
    tm_http_response_t *resp = tm_http_request(TM_HTTP_GET, url, mailinator_headers, NULL, 15);
...
static char *mailinator_first_text(const cJSON *obj, const char **keys, int key_count) {
    for (int i = 0; i < key_count; i++) {
        char *s = mailinator_json_to_string(cJSON_GetObjectItemCaseSensitive((cJSON*)obj, keys[i]));
        if (s && s[0]) {
            return s;
        }
        free(s);
...
static cJSON *mailinator_flatten_message(const cJSON *summary, const char *recipient,
                                         const cJSON *text_payload, const cJSON *html_payload,
    ...
    if (!id_item) id_item = cJSON_GetObjectItemCaseSensitive((cJSON*)summary, "messageId");
    id = mailinator_json_to_string(id_item);

    from = mailinator_json_to_string(cJSON_GetObjectItemCaseSensitive((cJSON*)summary, "from"));
    if (!from[0]) {
        free(from);
        from = mailinator_json_to_string(cJSON_GetObjectItemCaseSensitive((cJSON*)summary, "origfrom"));
    }

    to = mailinator_json_to_string(cJSON_GetObjectItemCaseSensitive((cJSON*)summary, "to"));
    if (!to[0]) {
        free(to);
        to = tm_strdup(recipient ? recipient : "");
    }

    subject = mailinator_json_to_string(cJSON_GetObjectItemCaseSensitive((cJSON*)summary, "subject"));

    text = mailinator_first_text(text_payload, (const char *[]){"text/plain", "text", "body"}, 3);
    html = mailinator_first_text(html_payload, (const char *[]){"text/html", "html", "body"}, 3);

    created_at = cJSON_Duplicate(cJSON_GetObjectItemCaseSensitive((cJSON*)summary, "time"), 1);
    if (!created_at) {
        created_at = cJSON_Duplicate(cJSON_GetObjectItemCaseSensitive((cJSON*)summary, "date"), 1);
    }
    if (!created_at) {
    ...
    if (attachments_payload) {
        cJSON *src = cJSON_GetObjectItemCaseSensitive((cJSON*)attachments_payload, "attachments");
        if (cJSON_IsArray(src)) {
            ...
            cJSON_ArrayForEach(att, src) {
                ...
                if (!item) continue;
                {
                    char *filename = mailinator_json_to_string(cJSON_GetObjectItemCaseSensitive((cJSON*)att, "name"));
                    cJSON_AddStringToObject(item, "filename", filename ? filename : "");
                    free(filename);
                }
                {
                    const cJSON *size = cJSON_GetObjectItemCaseSensitive((cJSON*)att, "size");
                    if (!size) size = cJSON_GetObjectItemCaseSensitive((cJSON*)att, "filesize");
                ...
                {
                    char *content_type = mailinator_json_to_string(cJSON_GetObjectItemCaseSensitive((cJSON*)att, "contentType"));
                    cJSON_AddStringToObject(item, "contentType", content_type ? content_type : "");
                    free(content_type);
                }
                {
                    char *url = mailinator_attachment_url(cJSON_GetObjectItemCaseSensitive((cJSON*)att, "downloadUrl"));
...
static cJSON *mailinator_fetch_detail(const char *message_id, const char *suffix) {
    char url[768];
...
tm_email_t* tm_provider_mailinator_get_emails(const char *email, int *count) {
    *count = -1;
    ...
    char url[768];
    snprintf(url, sizeof(url), "%s/api/v2/domains/%s/inboxes/%s",
        MAILINATOR_BASE, MAILINATOR_PUBLIC_DOMAIN, inbox);
    ...
    *count = n;
    ...
    for (int i = 0; i < n; i++) {
        ...
        const char *mid = TM_JSON_STR(cJSON_GetObjectItemCaseSensitive(msg, "id"), "");
        if (!mid[0]) {
        ...
        cJSON *html_payload = mid[0] ? mailinator_fetch_detail(mid, "texthtml") : NULL;
        cJSON *attachments_payload = mid[0] ? mailinator_fetch_detail(mid, "attachments") : NULL;

        cJSON *flat = mailinator_flatten_message(
            msg,
            email,
            text_payload,
            html_payload,
