/**
 * mailmomy 域名变体 mingyueming.shop
...
#define CHANNEL_MINGYUEMING_SHOP_DOMAIN "mingyueming.shop"

static void mingyueming_shop_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_mingyueming_shop_generate(void) {
  char local[11];
  mingyueming_shop_random_local(local, 10);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_MINGYUEMING_SHOP;
  {
    size_t cap =
        strlen(local) + strlen("@" CHANNEL_MINGYUEMING_SHOP_DOMAIN) + 1;
    info->email = (char *)malloc(cap);
    if (!info->email) {
      tm_free_email_info(info);
      return NULL;
    }
    snprintf(info->email, cap, "%s@%s", local, CHANNEL_MINGYUEMING_SHOP_DOMAIN);
  }
  /* token 即邮箱地址 */
