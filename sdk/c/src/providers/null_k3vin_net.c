/**
 * Mailinator 官方姊妹域名 null.k3vin.net
...
#define NULL_K3VIN_NET_DOMAIN "null.k3vin.net"

static void null_k3vin_net_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_null_k3vin_net_generate(void) {
  char local[13];
  null_k3vin_net_random_local(local, 12);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_NULL_K3VIN_NET;
  {
    size_t cap = strlen(local) + strlen("@" NULL_K3VIN_NET_DOMAIN) + 1;
    info->email = (char *)malloc(cap);
    if (!info->email) {
      tm_free_email_info(info);
      return NULL;
    }
    snprintf(info->email, cap, "%s@%s", local, NULL_K3VIN_NET_DOMAIN);
  }
  return info;
}
