...
static void fl_pick_domain(char domains[][256], int nd, const char *preferred,
                           char *out, size_t cap) {
  if (!out || cap < 2)
    return;
  out[0] = '\0';
...
tm_email_info_t *tm_provider_fake_legal_generate(const char *domain) {
  ...
  tm_http_response_t *resp = tm_http_request(
      TM_HTTP_GET, FL_BASE "/api/domains", fl_headers, NULL, 15);
  ...
  if (fl_stricmp(pickdom, "imgui.de") == 0 ||
      fl_stricmp(pickdom, "pulsewebmenu.de") == 0) {
    /* imgui-de / pulsewebmenu-de 用 POST 保根域 */
    char username[13];
    fl_random_username(username, 12);
    char body[512];
    snprintf(body, sizeof(body), "{\"username\":\"%s\",\"domain\":\"%s\"}",
             username, pickdom);
    const char *post_headers[] = {
        ...
        "Edg/146.0.0.0",
        NULL};
    r2 = tm_http_request(TM_HTTP_POST, FL_BASE "/api/inbox/custom",
  ...
  const char *addr =
      cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(cj, "address"));
  if (!addr || !addr[0]) {
    cJSON_Delete(cj);
    return NULL;
  }
...
tm_email_t *tm_provider_fake_legal_get_emails(const char *email, int *count) {
  *count = -1;
  ...
  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_GET, url, fl_headers, NULL, 15);
  if (!resp || resp->status < 200 || resp->status >= 300) {
    tm_http_response_free(resp);
    return NULL;
  }
