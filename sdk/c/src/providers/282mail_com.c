...
#define CHANNEL_282MAIL_COM_DOMAIN "282mail.com"

static void mm_282mail_com_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_282mail_com_generate(void) {
  char local[11];
  mm_282mail_com_random_local(local, 10);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_282MAIL_COM;
  {
    size_t cap = strlen(local) + strlen("@" CHANNEL_282MAIL_COM_DOMAIN) + 1;
    info->email = (char *)malloc(cap);
    if (!info->email) {
      tm_free_email_info(info);
      return NULL;
    }
    snprintf(info->email, cap, "%s@%s", local, CHANNEL_282MAIL_COM_DOMAIN);
  }
  /* token 即邮箱地址 */
