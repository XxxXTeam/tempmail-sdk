...
static int ms_fetch_domains(char domains[][128], int max_count) {
    tm_http_response_t *resp = tm_http_request(
        TM_HTTP_GET, MS_BASE "/api/domain", ms_headers, NULL, 15);
    if (!resp || resp->status < 200 || resp->status >= 300) {
        tm_http_response_free(resp);
        /* 使用备用域名 */
        int n = MS_FALLBACK_DOMAIN_COUNT < max_count ? MS_FALLBACK_DOMAIN_COUNT : max_count;
        for (int i = 0; i < n; i++) {
    ...
    if (n == 0) {
        /* API 返回空数组，使用备用域名 */
        n = MS_FALLBACK_DOMAIN_COUNT < max_count ? MS_FALLBACK_DOMAIN_COUNT : max_count;
        for (int i = 0; i < n; i++) {
            strncpy(domains[i], ms_fallback_domains[i], 127);
            domains[i][127] = '\0';
        }
...
tm_email_t* tm_provider_mail_sunls_get_emails(const char *email, int *count) {
    *count = -1;
    ...
    tm_http_response_t *resp = tm_http_request(TM_HTTP_GET, url, ms_headers, NULL, 15);
    if (!resp || resp->status < 200 || resp->status >= 300) {
        tm_http_response_free(resp);
        return NULL;
    }
    ...
    *count = n;
    ...
    for (int i = 0; i < n; i++) {
        ...
        int has_body = (cJSON_IsString(html_item) && html_item->valuestring && html_item->valuestring[0])
                    || (cJSON_IsString(text_item) && text_item->valuestring && text_item->valuestring[0]);
        ...
        if (!has_body && id[0]) {
            ...
            tm_http_response_t *detail_resp = tm_http_request(
                TM_HTTP_GET, detail_url, ms_headers, NULL, 15);
            if (detail_resp && detail_resp->status >= 200 && detail_resp->status < 300) {
                cJSON *detail = cJSON_Parse(detail_resp->body);
                if (detail && cJSON_IsObject(detail)) {
                    /* 将详情中的字段合并到列表项 */
                    const char *dhtml = TM_JSON_STR(
                        cJSON_GetObjectItemCaseSensitive(detail, "html"), "");
                    const char *dtext = TM_JSON_STR(
                        cJSON_GetObjectItemCaseSensitive(detail, "text"), "");
                    if (dhtml[0]) cJSON_AddStringToObject(raw, "html", dhtml);
                    if (dtext[0]) cJSON_AddStringToObject(raw, "text", dtext);
                    cJSON_Delete(detail);
                }
            }
