...
#define EBS_COM_AR_DOMAIN "ebs.com.ar"

static void ebs_com_ar_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_ebs_com_ar_generate(void) {
  char local[13];
  ebs_com_ar_random_local(local, 12);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_EBS_COM_AR;
  {
    size_t cap = strlen(local) + strlen("@" EBS_COM_AR_DOMAIN) + 1;
    info->email = (char *)malloc(cap);
    if (!info->email) {
      tm_free_email_info(info);
      return NULL;
    }
    snprintf(info->email, cap, "%s@%s", local, EBS_COM_AR_DOMAIN);
  }
  return info;
}
