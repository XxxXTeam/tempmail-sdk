...
static const char *mtm_headers[] = {"Content-Type: application/json",
...
tm_email_info_t *tm_provider_mail_tm_generate(void) {
  ...
  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_GET, MTM_BASE "/domains", mtm_headers, NULL, 15);
  ...
  const char *create_headers[] = {"Content-Type: application/ld+json",
                                  "Accept: application/json", NULL};
  resp = tm_http_request(TM_HTTP_POST, MTM_BASE "/accounts", create_headers,
                         body, 15);
  ...
  /* 4. 获取 Token */
  resp =
      tm_http_request(TM_HTTP_POST, MTM_BASE "/token", mtm_headers, body, 15);
  if (!resp || resp->status != 200) {
    tm_http_response_free(resp);
    return NULL;
  }
...
tm_email_t *tm_provider_mail_tm_get_emails(const char *token, const char *email,
  ...
  *count = -1;
  ...
  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_GET, MTM_BASE "/messages", headers, NULL, 15);
  if (!resp || resp->status != 200) {
    tm_http_response_free(resp);
    return NULL;
  }
