...
tm_email_info_t *tm_provider_harakirimail_generate(void) {
  ...
  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_GET, url, hk_headers, NULL, 15);
...
tm_email_t *tm_provider_harakirimail_get_emails(const char *email, int *count) {
  ...
  *count = n;
  ...
  for (int i = 0; i < n; i++) {
    ...
    const char *id =
        TM_JSON_STR(cJSON_GetObjectItemCaseSensitive(raw, "_id"), "");
    ...
    const char *recv =
        TM_JSON_STR(cJSON_GetObjectItemCaseSensitive(raw, "received"), "");
    if (recv[0])
      cJSON_AddStringToObject(raw, "date", recv);
