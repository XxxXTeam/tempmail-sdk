 ...
 */
static int teco_ck_parse_merge(teco_ck_t *tab, int n, int maxn, const char *hdr) {
    if (!hdr || !*hdr) return n;
    const char *p = hdr;
    while (*p) {
        while (*p == ' ' || *p == '\t' || *p == ';') p++;
...
static int teco_b64_encode(const unsigned char *in, size_t inlen, char *out, size_t outcap) {
...
static char *teco_build_token(const char *cookie_hdr, const char *addr) {
    ...
    if (teco_b64_encode((const unsigned char *)json, jl, tok + strlen(TECO_TOK_PREFIX), bcap - strlen(TECO_TOK_PREFIX)) != 0) {
        free(json);
        free(tok);
        return NULL;
    }
...
static int teco_parse_token(const char *tok, char **cookie_hdr, char **addr) {
    *cookie_hdr = NULL;
    *addr = NULL;
    if (!tok || strncmp(tok, TECO_TOK_PREFIX, strlen(TECO_TOK_PREFIX)) != 0) return -1;
    ...
    *addr = tm_strdup(cJSON_IsString(ja) && ja->valuestring ? ja->valuestring : "");
    cJSON_Delete(j);
...
tm_email_info_t *tm_provider_tempemail_co_generate(void) {
    ...
    tm_http_response_t *resp = tm_http_request(TM_HTTP_GET, TECO_BASE "/mail/random", hdrs, NULL, timeout);
    if (!resp || resp->status != 200 || !resp->body) {
        TM_LOG_ERR("[tempemail-co] 创建邮箱请求失败, status=%ld", resp ? resp->status : 0);
...
tm_email_t *tm_provider_tempemail_co_get_emails(const char *token, const char *email, int *count) {
    *count = -1;
    ...
    tm_http_response_t *resp = tm_http_request(TM_HTTP_GET, list_url, hdr_list, NULL, timeout);
    if (!resp || resp->status != 200 || !resp->body) {
        TM_LOG_ERR("[tempemail-co] 获取邮件列表请求失败, status=%ld", resp ? resp->status : 0);
        tm_http_response_free(resp);
        free(ck);
        free(addr);
        return NULL;
    ...
    if (id_count > TECO_MAX_MAILS) id_count = TECO_MAX_MAILS;
    ...
    for (int i = 0; i < id_count; i++) {
        char detail_url[256];
        snprintf(detail_url, sizeof(detail_url), TECO_BASE "/mail/info?id=%d", mail_ids[i]);
        ...
        /* 映射 fromAddress -> from */
        const cJSON *j_from_addr = cJSON_GetObjectItemCaseSensitive(j_mail, "fromAddress");
        ...
        const cJSON *j_from_name = cJSON_GetObjectItemCaseSensitive(j_mail, "fromName");
        if (cJSON_IsString(j_from_name) && j_from_name->valuestring && j_from_name->valuestring[0]) {
        ...
        /* 主题 */
        const cJSON *j_subject = cJSON_GetObjectItemCaseSensitive(j_mail, "subject");
        if (cJSON_IsString(j_subject) && j_subject->valuestring) {
            cJSON_AddStringToObject(raw, "subject", j_subject->valuestring);
        }

        /* 日期 */
        const cJSON *j_date = cJSON_GetObjectItemCaseSensitive(j_mail, "displayDate");
