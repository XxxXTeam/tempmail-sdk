 ...
 *   body: {}
 *   返回:
 * {"id":"...","address":"...@disposablemail.dev","token":"...","domain":"...","expiresAt":"...","createdAt":"..."}
 *
 * 获取邮件流程:
...
tm_email_info_t *tm_provider_disposablemail_app_generate(void) {
  ...
  tm_http_response_t *resp = tm_http_request(
      TM_HTTP_POST, DMA_BASE "/api/inbox", headers, "{}", timeout);

  if (!resp || resp->status != 200 || !resp->body) {
    TM_LOG_ERR("[disposablemail-app] 创建收件箱请求失败, status=%ld",
               resp ? resp->status : 0);
  ...
  if (!strchr(j_address->valuestring, '@')) {
    TM_LOG_ERR("[disposablemail-app] 返回的邮箱格式无效: %s",
  ...
  /* 提取可选字段 */
  const cJSON *j_expires_at =
      cJSON_GetObjectItemCaseSensitive(json, "expiresAt");
  const cJSON *j_created_at =
      cJSON_GetObjectItemCaseSensitive(json, "createdAt");
  ...
  info->expires_at = 0;
  info->created_at =
      tm_strdup((cJSON_IsString(j_created_at) && j_created_at->valuestring)
                    ? j_created_at->valuestring
                    : "");
...
tm_email_t *tm_provider_disposablemail_app_get_emails(const char *token,
  ...
  *count = -1;
  ...
  size_t url_len =
      strlen(DMA_BASE "/api/inbox/emails?token=") + strlen(token) + 1;
  ...
  /* 请求头 */
  const char *headers[] = {dma_ua, "Accept: application/json, text/plain, */*",
                           "Referer: https://disposablemail.app/", NULL};

  /* 发送请求 */
  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_GET, url, headers, NULL, timeout);
  free(url);

  if (!resp || resp->status != 200 || !resp->body) {
    TM_LOG_ERR("[disposablemail-app] 获取邮件请求失败, status=%ld",
               resp ? resp->status : 0);
    tm_http_response_free(resp);
    return NULL;
  }
  ...
  if (mail_count > DMA_MAX_MAILS)
    mail_count = DMA_MAX_MAILS;
