...
#define JAMA_TRENET_EU_DOMAIN "jama.trenet.eu"

static void jama_trenet_eu_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_jama_trenet_eu_generate(void) {
  char local[13];
  jama_trenet_eu_random_local(local, 12);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_JAMA_TRENET_EU;
  {
    size_t cap = strlen(local) + strlen("@" JAMA_TRENET_EU_DOMAIN) + 1;
    info->email = (char *)malloc(cap);
    if (!info->email) {
      tm_free_email_info(info);
      return NULL;
    }
    snprintf(info->email, cap, "%s@%s", local, JAMA_TRENET_EU_DOMAIN);
  }
  return info;
}
