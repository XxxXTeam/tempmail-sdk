...
static const char *one_sec_mail_generate_headers[] = {
    "Accept: application/json", "User-Agent: Mozilla/5.0", NULL};
...
tm_email_info_t *tm_provider_one_sec_mail_generate(void) {
  tm_http_response_t *resp =
      tm_http_request_ipv4(TM_HTTP_GET, ONE_SEC_MAIL_BASE "generate",
...
tm_email_t *tm_provider_one_sec_mail_get_emails(const char *token,
  ...
  *count = -1;
  ...
  size_t url_cap =
      strlen(ONE_SEC_MAIL_BASE) + strlen("emails?address=") + strlen(email) + 1;
  ...
  const char *headers[] = {"Accept: application/json",
                           "User-Agent: Mozilla/5.0", cookie_header, NULL};

  tm_http_response_t *resp =
      tm_http_request_ipv4(TM_HTTP_GET, url, headers, NULL, 15);
  free(url);
  free(cookie_header);
  if (!resp || resp->status < 200 || resp->status >= 300) {
    tm_http_response_free(resp);
