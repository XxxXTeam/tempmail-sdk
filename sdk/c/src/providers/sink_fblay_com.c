/**
 * Mailinator 官方姊妹域名 sink.fblay.com
...
#define SINK_FBLAY_COM_DOMAIN "sink.fblay.com"

static void sink_fblay_com_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_sink_fblay_com_generate(void) {
  char local[13];
  sink_fblay_com_random_local(local, 12);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_SINK_FBLAY_COM;
  {
    size_t cap = strlen(local) + strlen("@" SINK_FBLAY_COM_DOMAIN) + 1;
    info->email = (char *)malloc(cap);
    if (!info->email) {
      tm_free_email_info(info);
      return NULL;
    }
    snprintf(info->email, cap, "%s@%s", local, SINK_FBLAY_COM_DOMAIN);
  }
  return info;
}
