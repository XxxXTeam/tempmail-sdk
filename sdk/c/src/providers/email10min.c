...
static char *e10m_base64_decode(const char *input, size_t *out_len) {
  ...
  while (i < in_len) {
    ...
    int c = (input[i] == '=') ? 0 : b64_decode_char(input[i]);
    i++;
    int d = (input[i] == '=') ? 0 : b64_decode_char(input[i]);
    i++;
    if (a < 0 || b < 0) {
      free(output);
      return NULL;
    ...
    if (j < decoded_len)
      output[j++] = (char)((triple >> 8) & 0xFF);
    if (j < decoded_len)
      output[j++] = (char)(triple & 0xFF);
  }
...
static char *e10m_encode_token(const char *cookie, const char *csrf) {
  char raw[8192];
  int n =
      snprintf(raw, sizeof(raw), "{\"c\":\"%s\",\"t\":\"%s\"}", cookie, csrf);
...
/* 解码 token */
static int e10m_decode_token(const char *token, char **out_cookie,
                             char **out_csrf) {
  size_t prefix_len = strlen(E10M_TOK_PREFIX);
  if (strncmp(token, E10M_TOK_PREFIX, prefix_len) != 0)
    return 0;
  ...
  if (!cj)
    return 0;
  const char *c =
      cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(cj, "c"));
  const char *t =
      cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(cj, "t"));
  if (!c || !c[0] || !t || !t[0]) {
    cJSON_Delete(cj);
    return 0;
  }
...
static char *e10m_extract_email(const char *html) {
  ...
  while ((v = strstr(v, "value=\"")) != NULL) {
    ...
    if (end && end > v) {
      ...
      memcpy(buf, v, len);
      buf[len] = '\0';
      if (strchr(buf, '@') && !strstr(buf, "email10min") &&
          !strstr(buf, "example")) {
        return buf;
      }
      free(buf);
...
tm_email_info_t *tm_provider_email10min_generate(void) {
  tm_http_response_t *r = tm_http_request(TM_HTTP_GET, E10M_BASE "/zh",
                                          e10m_browser_hdrs, NULL, 15);
...
tm_email_t *tm_provider_email10min_get_emails(const char *token,
  ...
  *count = -1;
  ...
  int i = 0;
  for (const char **p = e10m_ajax_hdrs; *p; p++)
    post_hdrs[i++] = *p;
  post_hdrs[i++] = ck_line;
  post_hdrs[i] = NULL;

  tm_http_response_t *r =
      tm_http_request(TM_HTTP_POST, url, post_hdrs, body, 15);
  free(cookie);
  free(csrf);
  if (!r || r->status < 200 || r->status >= 300) {
    tm_http_response_free(r);
  ...
  *count = n;
  ...
  for (int j = 0; j < n; j++) {
    ...
    const char *to =
        cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(raw, "to"));
    const char *subj =
    ...
    if (!html_content)
      html_content = cJSON_GetStringValue(
    ...
    if (!date)
      date = cJSON_GetStringValue(
          cJSON_GetObjectItemCaseSensitive(raw, "created_at"));
