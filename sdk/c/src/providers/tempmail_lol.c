...
tm_email_info_t *tm_provider_tempmail_lol_generate(const char *domain) {
  char body[256];
  if (domain)
    snprintf(body, sizeof(body), "{\"domain\":\"%s\",\"captcha\":null}",
             domain);
  else
    snprintf(body, sizeof(body), "{\"domain\":null,\"captcha\":null}");

  tm_http_response_t *resp = tm_http_request(
      TM_HTTP_POST, TL_BASE "/inbox/create", tl_headers, body, 15);
...
tm_email_t *tm_provider_tempmail_lol_get_emails(const char *token,
  ...
  *count = -1;
  ...
  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_GET, url, tl_headers, NULL, 15);
  if (!resp || resp->status != 200) {
    tm_http_response_free(resp);
    return NULL;
  }
