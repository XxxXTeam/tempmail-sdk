...
static void mailcx_headers(const char **headers, char *client_line,
                           size_t client_line_cap, const char *client_id) {
  ...
  headers[3] = "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
...
static cJSON *mailcx_get_config(const char *client_id) {
  ...
  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_GET, MAILCX_BASE "/v1/config", headers, NULL, 15);
...
static char *mailcx_pick_domain(cJSON *config, const char *preferred) {
  ...
  if (preferred && preferred[0]) {
    ...
    if (len >= sizeof(want))
      len = sizeof(want) - 1;
    for (size_t i = 0; i < len; i++)
      want[i] = (char)tolower((unsigned char)p[i]);
    want[len] = '\0';
  }
  ...
  cJSON_ArrayForEach(item, domains) {
    ...
    if (!cJSON_IsString(domain) || !domain->valuestring ||
        !domain->valuestring[0])
      continue;
    if (want[0]) {
      char lower[256];
      ...
      if (len >= sizeof(lower))
        len = sizeof(lower) - 1;
      for (size_t i = 0; i < len; i++)
        lower[i] = (char)tolower((unsigned char)domain->valuestring[i]);
      lower[len] = '\0';
      if (strcmp(lower, want) == 0)
  ...
  cJSON_ArrayForEach(item, domains) {
    ...
    cJSON *domain = cJSON_GetObjectItemCaseSensitive(item, "domain");
    if (cJSON_IsTrue(is_default) && cJSON_IsString(domain) &&
        domain->valuestring && domain->valuestring[0]) {
      return tm_strdup(domain->valuestring);
    }
  }
...
static void mailcx_add_dup(cJSON *dst, const char *dst_key, cJSON *src,
                           const char *src_key) {
  cJSON *item = cJSON_GetObjectItemCaseSensitive(src, src_key);
  if (item)
    cJSON_AddItemToObject(dst, dst_key, cJSON_Duplicate(item, 1));
}
...
static cJSON *mailcx_flatten_list(cJSON *raw, const char *recipient) {
  ...
  cJSON_AddStringToObject(obj, "from",
                          mailcx_first_str(raw, "from_email", "from_name"));
...
static cJSON *mailcx_flatten_detail(cJSON *raw, const char *recipient) {
  ...
  cJSON_AddStringToObject(obj, "text",
                          mailcx_first_str(raw, "text_body", "preview_text"));
...
tm_email_t *tm_provider_mail_cx_get_emails(const char *client_id,
  ...
  *count = -1;
  if (!client_id || !client_id[0] || !email || !email[0])
    return NULL;
  ...
  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_GET, url, headers, NULL, 30);
  free(url);
  if (!resp)
    return NULL;
  if (resp->status == 204) {
