...
static const char *ta_easy_generate_headers[] = {
    ...
    "origin: " TA_EASY_ORIGIN, "referer: " TA_EASY_ORIGIN "/", NULL};
...
tm_email_info_t *tm_provider_ta_easy_generate(void) {
  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_POST, TA_EASY_API "/temp-email/address/new",
                      ta_easy_generate_headers, "", 15);
...
tm_email_t *tm_provider_ta_easy_get_emails(const char *token, const char *email,
  ...
  *count = -1;
  ...
  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_POST, TA_EASY_API "/temp-email/inbox/list",
                      ta_easy_json_headers, body, 15);
  free(body);
