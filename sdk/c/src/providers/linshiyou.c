...
static void tm_lsy_pick_div(const char *html, const char *cls, char *out,
                            size_t cap) {
  char marker[96];
...
static void tm_lsy_parse_time_iso(const char *s, char *out, size_t cap) {
  ...
  if (sscanf(s, "%d-%d-%d %d:%d:%d", &y, &mo, &d, &h, &mi, &se) == 6) {
    snprintf(out, cap, "%04d-%02d-%02dT%02d:%02d:%02d.000Z", y, mo, d, h, mi,
             se);
  }
}

static int tm_lsy_extract_list_id(const char *list_part, char *id_out,
                                  size_t cap) {
...
static tm_email_t *tm_lsy_parse_one(const char *list_part,
                                    const char *body_part,
                                    const char *recipient,
                                    tm_attachment_t **atts, int *att_count) {
  char idbuf[128];
  if (tm_lsy_extract_list_id(list_part, idbuf, sizeof(idbuf)) != 0)
    return NULL;
  ...
  tm_lsy_pick_div(body_part, "tmail-email-sender", from_body,
                  sizeof(from_body));
  tm_lsy_pick_div(body_part, "tmail-email-time", time_body, sizeof(time_body));
  tm_lsy_pick_div(body_part, "tmail-email-title", title_body,
                  sizeof(title_body));
  ...
  *atts = NULL;
  ...
  if (dl) {
    ...
    if (url) {
      ...
      tm_attachment_t *a =
          (tm_attachment_t *)calloc(1, sizeof(tm_attachment_t));
      if (a) {
        a->url = url;
...
tm_email_info_t *tm_provider_linshiyou_generate(void) {
  tm_http_response_t *resp = tm_http_request(
      TM_HTTP_GET, LSY_ORIGIN "/api/user?user", lsy_headers, NULL, 15);
  ...
  char nexus[128];
  if (!resp->cookies ||
      tm_lsy_extract_nexus(resp->cookies, nexus, sizeof(nexus)) != 0) {
    tm_http_response_free(resp);
    return NULL;
  }
...
tm_email_t *tm_provider_linshiyou_get_emails(const char *token,
  ...
  *count = -1;
  ...
  tm_http_response_t *resp = tm_http_request(
      TM_HTTP_GET, LSY_ORIGIN "/api/mail?unseen=1", headers, NULL, 15);
  ...
  for (;;) {
    ...
    if (seglen > 0) {
      ...
      if (seg) {
        ...
        int ac = 0;
        tm_email_t *one =
            tm_lsy_parse_one(list_part, body_part, email, &atts, &ac);
        if (one) {
          one->attachments = atts;
          one->attachment_count = ac;
          tm_email_t *nl =
              (tm_email_t *)realloc(list, (size_t)(n + 1) * sizeof(tm_email_t));
          if (nl) {
            list = nl;
            list[n] = *one;
            free(one);
