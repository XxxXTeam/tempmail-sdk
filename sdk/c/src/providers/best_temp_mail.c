 ...
 * 返回值需要 free
 */
static char *btm_build_token(const char *int_token, const char *id,
 ...
 * 输出的字符串需要 free
 */
static int btm_parse_token(const char *token, char **int_token, char **id,
                           char **update_tag) {
  *int_token = NULL;
  *id = NULL;
  *update_tag = NULL;
  ...
  const cJSON *j_update_tag =
      cJSON_GetObjectItemCaseSensitive(obj, "update_tag");
...
tm_email_info_t *tm_provider_best_temp_mail_generate(void) {
  ...
  tm_http_response_t *resp = tm_http_request(
      TM_HTTP_POST, BTM_BASE "/api/v3/createEmail", headers, body_str, timeout);
  free(body_str);

  if (!resp || resp->status != 200 || !resp->body) {
    TM_LOG_ERR("[best-temp-mail] createEmail 请求失败, status=%ld",
               resp ? resp->status : 0);
  ...
  const cJSON *j_update_tag =
      cJSON_GetObjectItemCaseSensitive(j_data, "update_tag");

  if (!cJSON_IsString(j_address) || !j_address->valuestring ||
      !j_address->valuestring[0] || !cJSON_IsString(j_id) ||
      !j_id->valuestring || !cJSON_IsString(j_update_tag) ||
      !j_update_tag->valuestring) {
  ...
  if (!strchr(j_address->valuestring, '@')) {
    TM_LOG_ERR("[best-temp-mail] 返回的邮箱格式无效: %s",
               j_address->valuestring);
    cJSON_Delete(json);
    return NULL;
  }
  ...
  /* 构建 token */
  char *token =
      btm_build_token(uuid, j_id->valuestring, j_update_tag->valuestring);
  if (!token) {
    cJSON_Delete(json);
    return NULL;
  }
...
tm_email_t *tm_provider_best_temp_mail_get_emails(const char *token,
  ...
  *count = -1;
  ...
  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_POST, BTM_BASE "/api/v3/getEmailList", headers,
                      body_str, timeout);
  free(body_str);

  if (!resp || resp->status != 200 || !resp->body) {
    TM_LOG_ERR("[best-temp-mail] getEmailList 请求失败, status=%ld",
               resp ? resp->status : 0);
    tm_http_response_free(resp);
    return NULL;
  }
  ...
  /* 检查是否有新邮件 */
  const cJSON *j_has_new =
      cJSON_GetObjectItemCaseSensitive(j_data, "hasNewEmail");
  ...
  if (mail_count > BTM_MAX_MAILS)
    mail_count = BTM_MAX_MAILS;
