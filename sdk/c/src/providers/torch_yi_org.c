/**
 * Mailinator 官方姊妹域名 torch.yi.org
...
#define TORCH_YI_ORG_DOMAIN "torch.yi.org"

static void torch_yi_org_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_torch_yi_org_generate(void) {
  char local[13];
  torch_yi_org_random_local(local, 12);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_TORCH_YI_ORG;
  {
    size_t cap = strlen(local) + strlen("@" TORCH_YI_ORG_DOMAIN) + 1;
    info->email = (char *)malloc(cap);
    if (!info->email) {
      tm_free_email_info(info);
      return NULL;
    }
    snprintf(info->email, cap, "%s@%s", local, TORCH_YI_ORG_DOMAIN);
  }
  return info;
}
