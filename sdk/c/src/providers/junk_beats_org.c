/**
 * Mailinator 官方姊妹域名 junk.beats.org
...
#define JUNK_BEATS_ORG_DOMAIN "junk.beats.org"

static void junk_beats_org_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_junk_beats_org_generate(void) {
  char local[13];
  junk_beats_org_random_local(local, 12);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_JUNK_BEATS_ORG;
  {
    size_t cap = strlen(local) + strlen("@" JUNK_BEATS_ORG_DOMAIN) + 1;
    info->email = (char *)malloc(cap);
    if (!info->email) {
      tm_free_email_info(info);
      return NULL;
    }
    snprintf(info->email, cap, "%s@%s", local, JUNK_BEATS_ORG_DOMAIN);
  }
  return info;
}
