...
static const char *awa_headers[] = {
    ...
    "sec-ch-ua: \"Not(A:Brand\";v=\"8\", \"Chromium\";v=\"144\", \"Microsoft "
    "Edge\";v=\"144\"",
    "sec-ch-ua-mobile: ?0",
...
tm_email_info_t *tm_provider_awamail_generate(void) {
  tm_http_response_t *resp = tm_http_request(
      TM_HTTP_POST, AWA_BASE "/change_mailbox", awa_headers, "", 15);
  ...
  info->email = tm_strdup(cJSON_GetStringValue(
      cJSON_GetObjectItemCaseSensitive(data, "email_address")));
  info->token = session;
  cJSON_Delete(json);
  return info;
}
...
tm_email_t *tm_provider_awamail_get_emails(const char *token, const char *email,
  ...
  *count = -1;
  ...
  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_GET, AWA_BASE "/get_emails", headers, NULL, 15);
  if (!resp || resp->status != 200) {
    tm_http_response_free(resp);
    return NULL;
  }
