/**
 * Mailinator 官方姊妹域名 spam.no-ip.net
...
#define SPAM_NO_IP_NET_DOMAIN "spam.no-ip.net"

static void spam_no_ip_net_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_spam_no_ip_net_generate(void) {
  char local[13];
  spam_no_ip_net_random_local(local, 12);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_SPAM_NO_IP_NET;
  {
    size_t cap = strlen(local) + strlen("@" SPAM_NO_IP_NET_DOMAIN) + 1;
    info->email = (char *)malloc(cap);
    if (!info->email) {
      tm_free_email_info(info);
      return NULL;
    }
    snprintf(info->email, cap, "%s@%s", local, SPAM_NO_IP_NET_DOMAIN);
  }
  return info;
}
