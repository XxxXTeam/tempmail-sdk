...
tm_email_info_t *tm_provider_24mail_chacuo_generate(void) {
    ...
    tm_http_response_t *r = tm_http_request(
        TM_HTTP_POST, CHACUO_BASE "/", chacuo_hdrs, body, 15);
...
tm_email_t *tm_provider_24mail_chacuo_get_emails(const char *email, int *count) {
    *count = -1;
    ...
    if (!status_item || !cJSON_IsNumber(status_item) || status_item->valueint != 1) {
        cJSON_Delete(cj);
        *count = 0;
        return NULL;
    }
    ...
    *count = n;
    ...
    for (int i = 0; i < n; i++) {
        ...
        const char *subj = cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(item, "SUBJECT"));
        const char *content = cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(item, "CONTENT"));
        const char *time_str = cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(item, "TIME"));
