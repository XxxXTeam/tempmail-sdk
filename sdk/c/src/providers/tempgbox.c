...
static tm_http_response_t *tgbox_post(const char *route, const char *device_id, const char *body) {
    char url[256];
    char ip[32];
    char device_hdr[96];
    char xff[64];
...
tm_email_info_t *tm_provider_tempgbox_generate(void) {
    ...
    tm_http_response_t *resp = tgbox_post("generate", device_id, "{\"variant\":\"googlemail\"}");
    ...
    if (!cJSON_IsObject(alias)) alias = json;
    const char *email = cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(alias, "email"));
    if (!email) email = cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(alias, "alias"));
    if (!email) {
        cJSON_Delete(json);
        return NULL;
    }
    ...
    const char *created = cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(alias, "created_at"));
    if (created) info->created_at = tm_strdup(created);
    cJSON_Delete(json);
    return info;
}
...
tm_email_t *tm_provider_tempgbox_get_emails(const char *device_id, const char *email, int *count) {
    *count = -1;
    if (!device_id || !device_id[0] || !email || !email[0]) return NULL;
    char body[512];
    ...
    *count = n;
    ...
    for (int i = 0; i < n; i++) {
        ...
        const char *id = cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(raw, "id"));
        if (!id) id = cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(raw, "message_id"));
        ...
        if (!text) text = cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(raw, "body_text"));
        cJSON_AddStringToObject(flat, "text", text ? text : "");
        const char *html = cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(raw, "html"));
        if (!html) html = cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(raw, "body_html"));
        ...
        if (!date) date = cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(raw, "received_at"));
        cJSON_AddStringToObject(flat, "date", date ? date : "");
        cJSON *atts = cJSON_GetObjectItemCaseSensitive(raw, "attachments");
        if (!atts) atts = cJSON_GetObjectItemCaseSensitive(raw, "attachments_info");
        if (atts) cJSON_AddItemToObject(flat, "attachments", cJSON_Duplicate(atts, 1));
