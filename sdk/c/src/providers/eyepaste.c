 ...
 */
static void ep_parse_description_header(const char *desc, char *from_out,
                                        size_t from_cap, char *subject_out,
                                        size_t subject_cap, char *date_out,
                                        size_t date_cap) {
  from_out[0] = '\0';
  subject_out[0] = '\0';
  date_out[0] = '\0';
  ...
  if (!subj_ptr)
    subj_ptr = strstr(desc, "Subject :");
  if (subj_ptr) {
    subj_ptr += 8;
    while (*subj_ptr == ' ' || *subj_ptr == ':')
      subj_ptr++;
    const char *end = strstr(subj_ptr, " Date:");
    if (!end)
      end = strstr(subj_ptr, " Date :");
    if (end) {
  ...
  if (date_ptr) {
    ...
    if (!end)
      end = strstr(date_ptr, "</p>");
    if (!end)
      end = date_ptr + strlen(date_ptr);
...
tm_email_t *tm_provider_eyepaste_get_emails(const char *email, int *count) {
  *count = -1;
  ...
  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_GET, url, ep_headers, NULL, 15);
  ...
  *count = n;
  ...
  for (int i = 0; i < n; i++) {
    ...
    ep_parse_description_header(desc, from, sizeof(from), subject,
                                sizeof(subject), date, sizeof(date));
