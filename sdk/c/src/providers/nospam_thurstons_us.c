...
#define NOSPAM_THURSTONS_US_DOMAIN "nospam.thurstons.us"

static void nospam_thurstons_us_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_nospam_thurstons_us_generate(void) {
  char local[13];
  nospam_thurstons_us_random_local(local, 12);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_NOSPAM_THURSTONS_US;
  {
    size_t cap = strlen(local) + strlen("@" NOSPAM_THURSTONS_US_DOMAIN) + 1;
    info->email = (char *)malloc(cap);
    if (!info->email) {
      tm_free_email_info(info);
      return NULL;
    }
    snprintf(info->email, cap, "%s@%s", local, NOSPAM_THURSTONS_US_DOMAIN);
  }
  return info;
}
