...
static const char *tmp_headers[] = {
    ...
    "Referer: https://tempmail.plus/", "Origin: https://tempmail.plus", NULL};
...
tm_email_info_t *tm_provider_tempmail_plus_generate(const char *domain,
  ...
  const char *selected_domain =
      (domain && domain[0]) ? domain : TMP_DEFAULT_DOMAIN;
  ...
  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_GET, url, tmp_headers, NULL, 15);
...
/* 获取单封邮件正文 */
static void tmp_fetch_body(int mail_id, const char *email, char **html_out,
                           char **text_out) {
  *html_out = NULL;
  *text_out = NULL;
  if (mail_id == 0)
...
tm_email_t *tm_provider_tempmail_plus_get_emails(const char *email,
                                                 int *count) {
  *count = -1;
  if (!email || !email[0])
    return NULL;
  ...
  *count = n;
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (text_body)
      cJSON_AddStringToObject(raw, "text", text_body);
    ...
    const char *from_mail =
        TM_JSON_STR(cJSON_GetObjectItemCaseSensitive(raw, "from_mail"), "");
    if (from_mail[0])
      cJSON_AddStringToObject(raw, "from", from_mail);

    const char *time_str =
        TM_JSON_STR(cJSON_GetObjectItemCaseSensitive(raw, "time"), "");
    if (time_str[0])
      cJSON_AddStringToObject(raw, "date", time_str);
