...
#define SPAM_WTF_AT_DOMAIN "spam.wtf.at"

static void spam_wtf_at_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_spam_wtf_at_generate(void) {
  char local[13];
  spam_wtf_at_random_local(local, 12);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_SPAM_WTF_AT;
  {
    size_t cap = strlen(local) + strlen("@" SPAM_WTF_AT_DOMAIN) + 1;
    info->email = (char *)malloc(cap);
    if (!info->email) {
      tm_free_email_info(info);
      return NULL;
    }
    snprintf(info->email, cap, "%s@%s", local, SPAM_WTF_AT_DOMAIN);
  }
  return info;
}
