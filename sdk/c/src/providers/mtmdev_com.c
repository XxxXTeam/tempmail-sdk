...
#define MTMDEV_COM_DOMAIN "mtmdev.com"

static void mtmdev_com_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_mtmdev_com_generate(void) {
  char local[13];
  mtmdev_com_random_local(local, 12);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_MTMDEV_COM;
  {
    size_t cap = strlen(local) + strlen("@" MTMDEV_COM_DOMAIN) + 1;
    info->email = (char *)malloc(cap);
    if (!info->email) {
      tm_free_email_info(info);
      return NULL;
    }
    snprintf(info->email, cap, "%s@%s", local, MTMDEV_COM_DOMAIN);
  }
  return info;
}
