/**
 * Mailinator 姊妹域名 mailinatorzz.mooo.com
...
#define CHANNEL_MAILINATORZZ_MOOO_COM_DOMAIN "mailinatorzz.mooo.com"

static void mailinatorzz_mooo_com_random_local(char *buf, int len) {
  static int seeded = 0;
  if (!seeded) {
...
tm_email_info_t *tm_provider_mailinatorzz_mooo_com_generate(void) {
  char local[13];
  mailinatorzz_mooo_com_random_local(local, 12);

  tm_email_info_t *info = tm_email_info_new();
  if (!info) {
    return NULL;
  }
  info->channel = CHANNEL_MAILINATORZZ_MOOO_COM;
  {
    size_t cap =
        strlen(local) + strlen("@" CHANNEL_MAILINATORZZ_MOOO_COM_DOMAIN) + 1;
    ...
    snprintf(info->email, cap, "%s@%s", local,
             CHANNEL_MAILINATORZZ_MOOO_COM_DOMAIN);
  }
  return info;
}
