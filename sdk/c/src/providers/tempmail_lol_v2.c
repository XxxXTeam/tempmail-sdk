...
tm_email_info_t *tm_provider_tempmail_lol_v2_generate(void) {
  tm_http_response_t *resp = tm_http_request(TM_HTTP_GET, TLV2_BASE "/generate",
                                             tlv2_headers, NULL, 15);
  ...
  const char *addr =
      cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(json, "address"));
  const char *token =
...
tm_email_t *tm_provider_tempmail_lol_v2_get_emails(const char *token,
  ...
  *count = -1;
  ...
  tm_http_response_t *resp =
      tm_http_request(TM_HTTP_GET, url, tlv2_headers, NULL, 15);
  if (!resp || resp->status != 200) {
    tm_http_response_free(resp);
    return NULL;
  }
  ...
  *count = n;
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (!id)
      id = cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(item, "_id"));
    cJSON_AddStringToObject(flat, "id", id ? id : "");

    const char *from =
        cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(item, "from"));
    if (!from)
      from = cJSON_GetStringValue(
          cJSON_GetObjectItemCaseSensitive(item, "sender"));
    ...
    const char *body_str =
        cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(item, "body"));
    const char *text_str =
        cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(item, "text"));
    const char *html_str =
        cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(item, "html"));

    cJSON_AddStringToObject(flat, "text",
                            body_str ? body_str : (text_str ? text_str : ""));
    cJSON_AddStringToObject(flat, "html",
                            html_str ? html_str : (body_str ? body_str : ""));

    const char *date =
        cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(item, "date"));
    if (!date)
      date = cJSON_GetStringValue(
          cJSON_GetObjectItemCaseSensitive(item, "receivedAt"));
