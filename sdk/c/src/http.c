...
void tm_set_config(const tm_config_t *config) {
  ...
  if (g_telemetry_url_copy) {
    free(g_telemetry_url_copy);
    g_telemetry_url_copy = NULL;
  }
  if (!config) {
  ...
  TM_LOG_INF(
      "SDK 配置已更新: proxy=%s timeout=%d insecure=%d telemetry_enabled=%d",
      g_config.proxy ? g_config.proxy : "(none)", g_config.timeout_secs,
      g_config.insecure, (int)g_config.telemetry_enabled);
}

const tm_config_t *tm_get_config(void) { return &g_config; }
...
static size_t header_callback(char *buffer, size_t size, size_t nitems,
  ...
  if (strncasecmp(buffer, "set-cookie:", 11) == 0) {
    ...
    while (nvlen > 0 && (val[nvlen - 1] == ' ' || val[nvlen - 1] == '\t'))
      nvlen--;
 ...
 */
static tm_http_response_t *
tm_http_perform(tm_http_method_t method, const char *url, const char **headers,
                const char *body, int timeout_secs, int skip_cert_verify_force,
                int force_ipv4) {
  tm_http_response_t *resp =
      (tm_http_response_t *)calloc(1, sizeof(tm_http_response_t));
  if (!resp)
    return NULL;
...
tm_http_response_t *tm_http_request(tm_http_method_t method, const char *url,
...
static void load_env_config(void) {
  ...
  if (timeout && timeout[0]) {
    int t = atoi(timeout);
    if (t > 0)
      g_config.timeout_secs = t;
  }
  const char *insecure = getenv("TEMPMAIL_INSECURE");
  if (insecure &&
      (insecure[0] == '1' || strncasecmp(insecure, "true", 4) == 0)) {
    g_config.insecure = true;
  }
  const char *te = getenv("TEMPMAIL_TELEMETRY_ENABLED");
  if (te && te[0]) {
    if (strncasecmp(te, "false", 5) == 0 || strcmp(te, "0") == 0 ||
        strncasecmp(te, "no", 3) == 0) {
      g_config.telemetry_enabled = false;
    } else if (strncasecmp(te, "true", 4) == 0 || strcmp(te, "1") == 0 ||
               strncasecmp(te, "yes", 3) == 0) {
      g_config.telemetry_enabled = true;
    }
  }
