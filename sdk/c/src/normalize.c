...
static char *tm_normalize_date_json(const cJSON *raw) {
  const char *k1[] = {"received_at", "receivedAt", "created_at", "createdAt",
                      "date"};
  for (int i = 0; i < 5; i++) {
...
tm_email_t tm_normalize_email(const cJSON *raw, const char *recipient) {
  ...
  /* ID */
  const char *id_keys[] = {"id",        "eid",       "_id",
                           "mailboxId", "messageId", "mail_id"};
  email.id = tm_json_get_str(raw, id_keys, 6);

  /* From */
  const char *from_keys[] = {"from_addr",   "from_address", "fromAddress",
                             "mail_sender", "sender",       "address_from",
                             "from_email",  "from",         "messageFrom"};
  email.from_addr = tm_json_get_str(raw, from_keys, 9);

  /* To */
  const char *to_keys[] = {"to",      "to_address",    "toAddress",
                           "name_to", "email_address", "address"};
  ...
  /* Text */
  const char *text_keys[] = {"text",           "text_body",    "preview_text",
                             "mail_body_text", "body",         "content",
                             "body_text",      "text_content", "description"};
  ...
  if (email.text && email.text[0] != '\0' &&
    ...
    if (strncmp(pp, "<!doctype html", 14) == 0 ||
        strncmp(pp, "<html", 5) == 0 || strncmp(pp, "<body", 5) == 0) {
      is_html = 1;
    } else if ((strstr(email.text, "<div") && strstr(email.text, "</div>")) ||
               (strstr(email.text, "<DIV") && strstr(email.text, "</DIV>")) ||
               (strstr(email.text, "<table") &&
                strstr(email.text, "</table>")) ||
               (strstr(email.text, "<TABLE") &&
                strstr(email.text, "</TABLE>")) ||
  ...
  if ((!email.text || email.text[0] == '\0') && email.html &&
      email.html[0] != '\0') {
  ...
  if (cJSON_IsArray(att_arr)) {
    ...
    if (count > 0) {
      email.attachments =
          (tm_attachment_t *)calloc(count, sizeof(tm_attachment_t));
      if (email.attachments) {
        ...
        cJSON_ArrayForEach(a, att_arr) {
          ...
          if (!sz)
            sz = cJSON_GetObjectItemCaseSensitive(a, "filesize");
          email.attachments[idx].size =
              sz && cJSON_IsNumber(sz) ? (long)sz->valuedouble : -1;

          const char *ct_keys[] = {"contentType", "content_type", "mimeType",
                                   "mime_type"};
