...
static int should_retry(const char *err) {
  if (!err)
    return 0;
  const char *keywords[] = {
      "connection", "timeout", "timed out", "dns",     "eof", "broken pipe",
      "refused",    "reset",   "ssl",       "network", ": 4", ": 5",
  };
  /* 简单的大小写无关子串匹配 */
  for (int i = 0; i < (int)(sizeof(keywords) / sizeof(keywords[0])); i++) {
    if (strstr(err, keywords[i]))
      return 1;
  }
  return 0;
}

tm_email_info_t *tm_retry_generate(tm_email_info_t *(*fn)(void),
  ...
  for (int attempt = 0; attempt <= c.max_retries; attempt++) {
    ...
    if (delay > c.max_delay_ms)
      delay = c.max_delay_ms;
