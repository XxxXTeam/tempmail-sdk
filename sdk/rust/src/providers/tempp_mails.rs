/*!
 * tempp-mails 渠道实现
 * 网站: https://tempp-mails.com
 *
 * Laravel + CSRF 后端，与 tempmailten 同构。
 * 流程:
...
const BASE_URL: &str = "https://tempp-mails.com";
...
/// 编码会话信息为 token 字符串
fn encode_sess(s: &TemppMailsSess) -> Result<String, String> {
    serde_json::to_string(s).map_err(|e| format!("tempp-mails: token 序列化失败: {}", e))
}

/// 从 token 字符串解码会话信息
fn decode_sess(tok: &str) -> Result<TemppMailsSess, String> {
    let s: TemppMailsSess =
        serde_json::from_str(tok).map_err(|_| "tempp-mails: token 反序列化失败".to_string())?;
    if s.csrf.is_empty() {
        return Err("tempp-mails: token 中 csrf 为空".into());
    }
    Ok(s)
}
...
async fn post_messages(
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| format!("tempp-mails: 请求 /messages 失败: {}", e))?;

    if !resp.status().is_success() {
        return Err(format!("tempp-mails messages: {}", resp.status()));
    }
    ...
    let text = resp
        .text()
        .await
        .map_err(|e| format!("tempp-mails: 读取 /messages 响应失败: {}", e))?;
    let data: Value = serde_json::from_str(text.trim())
        .map_err(|e| format!("tempp-mails: 解析 /messages 响应失败: {}", e))?;
    Ok((data, merged))
}
...
pub fn generate_email() -> Result<EmailInfo, String> {
    block_on(async {
        ...
        let resp = client
            ...
            .send()
            .await
            .map_err(|e| format!("tempp-mails: 获取首页失败: {}", e))?;

        if !resp.status().is_success() {
            return Err(format!("tempp-mails: 首页返回 HTTP {}", resp.status()));
        }
        ...
        let html = resp
            .text()
            .await
            .map_err(|e| format!("tempp-mails: 读取首页响应失败: {}", e))?;
        ...
        let csrf = CSRF_RE
            ...
            .filter(|s| !s.is_empty())
            .ok_or_else(|| "tempp-mails: 未能从首页提取 CSRF token".to_string())?;
        ...
        if mailbox.is_empty() || !mailbox.contains('@') {
            return Err("tempp-mails: 响应中未包含有效的 mailbox 字段".into());
        }

        let tok = encode_sess(&TemppMailsSess {
            csrf,
            cookie: cookie_hdr,
        })?;
...
pub fn get_emails(token: &str, email: &str) -> Result<Vec<Email>, String> {
    let em = email.trim();
    if em.is_empty() {
        return Err("tempp-mails: 邮箱地址为空".into());
    }
