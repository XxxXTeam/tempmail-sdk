 ...
 * Lroid 渠道实现
 * 网站: https://lroid.com
 * 模式: 通过 kontrol API 创建邮箱和获取邮件
 * 域名: yevme.com
 *
 * 流程:
 *   1. GET https://lroid.com 获取首页 HTML 和 session cookies
 *   2. 从 HTML 中提取 <input id="eposta_adres" value="xxx@yevme.com"> 获取邮箱地址
 *   3. GET https://lroid.com/en/api-kontrol/ 通过 kontrol API 获取邮件列表
 */
...
const BASE_URL: &str = "https://lroid.com";
...
fn init_session() -> Result<(String, String), String> {
    block_on(async {
        let resp = http_client_no_cookie_jar()
            ...
            .send()
            .await
            .map_err(|e| format!("lroid: 初始化会话失败: {}", e))?;

        let cookies = extract_cookies(resp.headers());
        if cookies.is_empty() {
            return Err("lroid: 未获取到 session cookies".into());
        }

        let html = resp
            .text()
            .await
            .map_err(|e| format!("lroid: 读取首页失败: {}", e))?;
        ...
        let email = re
            ...
            .ok_or_else(|| "lroid: 未从 HTML 中提取到邮箱地址".to_string())?;

        if email.is_empty() || !email.contains('@') {
            return Err(format!("lroid: 提取到的邮箱地址无效: {}", email));
        }
...
pub fn get_emails(token: &str, email: &str) -> Result<Vec<Email>, String> {
    if token.is_empty() {
        return Err("lroid: token(cookies) 不能为空".into());
    }
    ...
    block_on(async {
        let resp = http_client_no_cookie_jar()
            ...
            .send()
            .await
            .map_err(|e| format!("lroid: kontrol API 请求失败: {}", e))?;

        if !resp.status().is_success() {
            return Err(format!("lroid: kontrol API 返回 {}", resp.status()));
        }

        let body = resp
            .text()
            .await
            .map_err(|e| format!("lroid: 读取 kontrol 响应失败: {}", e))?;
