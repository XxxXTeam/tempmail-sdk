...
/// 从 token 字符串解码会话信息
fn decode_sess(tok: &str) -> Result<EmailtempOrgSess, String> {
    let s: EmailtempOrgSess =
        serde_json::from_str(tok).map_err(|_| "emailtemp-org: token 反序列化失败".to_string())?;
    if s.csrf.is_empty() {
        return Err("emailtemp-org: token 中 csrf 为空".into());
    }
    Ok(s)
}
...
pub fn generate_email() -> Result<EmailInfo, String> {
    block_on(async {
        ...
        let csrf = CSRF_RE
            ...
            .filter(|s| !s.is_empty())
            .ok_or_else(|| "emailtemp-org: 未能从首页提取 CSRF token".to_string())?;
        ...
        if mailbox.is_empty() || !mailbox.contains('@') {
            return Err("emailtemp-org: 响应中未包含有效的 mailbox 字段".into());
        }

        let tok = encode_sess(&EmailtempOrgSess {
            csrf,
            cookie: cookie_hdr,
        })?;
...
pub fn get_emails(token: &str, email: &str) -> Result<Vec<Email>, String> {
    let em = email.trim();
    if em.is_empty() {
        return Err("emailtemp-org: 邮箱地址为空".into());
    }
