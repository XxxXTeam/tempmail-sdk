...
(async () => {
  ...
  await sendSmtp(
    ...
    `Plus Test [${mk}]`,
    `纯文本正文 ${mk}`,
    `<h1>HTML正文</h1><p>marker: <b>${mk}</b></p>`,
  );
  ...
  for (let j = 0; j < 10; j++) {
    ...
    if (!r.success) {
      console.log(`  poll ${j + 1}: fail`);
      continue;
    }
    const f = r.emails.find(
      (e) =>
        e.subject?.includes(mk) || e.text?.includes(mk) || e.html?.includes(mk),
    );
    if (f) {
      console.log(`✅ 收到邮件!`);
      console.log(`  Subject: ${f.subject}`);
      console.log(
        `  Text长度: ${(f.text || "").length}  预览: ${(f.text || "").substring(0, 60)}`,
      );
      console.log(
        `  HTML长度: ${(f.html || "").length}  预览: ${(f.html || "").substring(0, 60)}`,
      );
