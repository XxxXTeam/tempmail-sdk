...
async function main() {
  ...
  try {
    ...
    console.log(
      `getEmails result: success=${r.success} count=${r.emails.length}`,
    );
  } catch (e: any) {
  ...
  try {
    ...
    console.log(
      `getEmails result: success=${r2.success} count=${r2.emails.length}`,
    );
  } catch (e: any) {
    console.log(`getEmails THREW: ${e.message}`);
    console.log(e.stack);
