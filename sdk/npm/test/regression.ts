...
interface Result {
  channel: string; genOk: boolean; email: string;
  getOk: boolean; smtpSent: boolean; smtpRecv: boolean;
  genErr: string; getErr: string; timeMs: number;
}

async function test(ch: Channel): Promise<Result> {
  const r: Result = { channel: ch, genOk: false, email: '', getOk: false,
    smtpSent: false, smtpRecv: false, genErr: '', getErr: '', timeMs: 0 };
  const t0 = Date.now();
  console.log(`\n[${ch}]`);
  try {
    const client = new TempEmailClient();
    const info = await client.generate({ channel: ch, channelFallback: false });
    if (!info) { r.genErr = 'null'; console.log('  ❌ gen=null'); r.timeMs = Date.now()-t0; return r; }
    r.genOk = true; r.email = info.email;
    console.log(`  ✅ ${info.email}`);
    try {
      const res = await client.getEmails();
      r.getOk = res.success;
      console.log(`  ${res.success ? '✅' : '⚠️'} getEmails=${res.success} (${res.emails.length})`);
    } catch (e: any) { r.getErr = e.message; console.log(`  ❌ getEmails: ${e.message.substring(0,50)}`); }
    ...
    if (r.smtpSent) {
      for (let i = 0; i < 8; i++) {
        ...
        try {
          const res = await client.getEmails();
          if (res.success && res.emails.some(e =>
            e.subject?.includes(marker) || e.text?.includes(marker) || e.html?.includes(marker))) {
            r.smtpRecv = true; break;
          }
        } catch {}
      }
      console.log(`  ${r.smtpRecv ? '✅' : '⚠️'} SMTP recv=${r.smtpRecv}`);
    }
  } catch (e: any) { r.genErr = e.message; console.log(`  ❌ ${e.message.substring(0,60)}`); }
...
async function main() {
  ...
  console.log('\n渠道'.padEnd(22) + '生成'.padEnd(6) + '获取'.padEnd(6) + 'SMTP'.padEnd(6) + '耗时');
  ...
  if (fail.length) {
    console.log(`\n❌ 失败渠道:`);
    for (const r of fail) console.log(`  ${r.channel}: ${r.genErr.substring(0,80)}`);
  }
}
