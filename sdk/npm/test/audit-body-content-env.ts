...
async function send(
  to: string,
  marker: string,
): Promise<{ textMarker: string; htmlMarker: string }> {
...
async function testChannel(channel: Channel): Promise<void> {
  ...
  console.log(
    JSON.stringify({ channel, step: "sent", email: info.email, marker }),
  );

  for (let round = 1; round <= 24; round++) {
    await sleep(5000);
    const result = await client.getEmails({
      retry: { maxRetries: 0, timeout: 15000 },
    });
    ...
    console.log(
      JSON.stringify({
        ...
        textOk:
          found.text.includes(markers.textMarker) ||
          found.text.includes(markers.htmlMarker),
        htmlOk:
          found.html.includes(markers.htmlMarker) ||
          found.html.includes(markers.textMarker),
        textDerivedFromHtml:
          !found.text.includes(markers.textMarker) &&
          found.text.includes(markers.htmlMarker),
        htmlDerivedFromText:
...
main().catch((err) => {
  console.error(
    JSON.stringify({ step: "error", message: err?.message || String(err) }),
  );
  process.exit(1);
});
