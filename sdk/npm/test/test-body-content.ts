...
async function send(to: string, marker: string): Promise<boolean> {
  try {
    return await sendSmtp(
      ...
      `Body [${marker}]`,
      `纯文本正文 marker=${marker}`,
      `<h1>HTML正文</h1><p>marker=<b>${marker}</b></p>`,
    );
  } catch {
    return false;
  }
  ...
  try {
    ...
    for (let j = 0; j < 10; j++) {
      ...
      try {
        ...
        const f = res.emails.find(
          (e) =>
            e.subject?.includes(mk) ||
            e.text?.includes(mk) ||
            e.html?.includes(mk),
        );
        if (f) {
          r.recv = true;
          r.subj = f.subject || "";
          r.textLen = (f.text || "").length;
          r.htmlLen = (f.html || "").length;
    ...
    r.issue = `err:${e.message?.substring(0, 40)}`;
  }
  r.time = Date.now() - t0;
  return r;
}
...
async function main() {
  console.log(
    `全渠道正文内容测试 (${ALL.length}渠道) ${new Date().toISOString()}\n`,
  );
  const results: R[] = [];
  for (const ch of ALL) {
    ...
    console.log(
      r.gen
        ? r.recv
          ? `✅ ${r.issue} text=${r.textLen} html=${r.htmlLen}`
          : `⚠️ ${r.issue}`
        : `❌ ${r.issue}`,
    );
    results.push(r);
  ...
  for (const r of results) {
    ...
    console.log(
      `${r.ch.padEnd(20)}${g.padEnd(5)}${s.padEnd(5)}${su.padEnd(9)}${tx.padEnd(12)}${ht.padEnd(12)}${r.issue}`,
    );
  }
