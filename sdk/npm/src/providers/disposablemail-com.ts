...
export async function generateEmail(): Promise<InternalEmailInfo> {
  ...
  const r2 = await fetchWithTimeout(`${BASE_URL}/index/index?csrf_token=${encodeURIComponent(csrf)}`, {
...
export async function getEmails(token: string, email: string): Promise<Email[]> {
  ...
  const r = await fetchWithTimeout(`${BASE_URL}/index/refresh`, { headers: ajaxHeaders(cookie) });
  if (!r.ok) {
...
async function fetchBody(cookie: string, id: string): Promise<string> {
  ...
  const r = await fetchWithTimeout(`${BASE_URL}/email/id/${id}`, { headers: ajaxHeaders(cookie) });
  if (!r.ok) {
    return '';
  }
  return await r.text();
