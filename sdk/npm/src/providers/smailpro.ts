 ...
 */
async function fetchPayload(targetURL: string, extra?: Record<string, string>): Promise<string> {
  ...
  const response = await fetchWithTimeout(reqURL, { method: 'GET', headers: COMMON_HEADERS });
  if (!response.ok) {
 ...
 */
async function callAPI(targetURL: string, extra: Record<string, string> | undefined, label: string): Promise<string> {
...
export async function getEmails(_token: string, email: string): Promise<Email[]> {
  ...
  const inboxBody = await callAPI(`${API_BASE_URL}/inbox`, { email: addr }, 'inbox');
 ...
 */
async function fetchMessage(email: string, mid: string): Promise<{ html: string; text: string }> {
  if (!mid) {
    return { html: '', text: '' };
  }
  try {
    const body = await callAPI(`${API_BASE_URL}/message`, { email, mid }, 'message');
