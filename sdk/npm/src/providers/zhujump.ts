...
function encodeToken(token: SessionToken): string {
  return TOKEN_PREFIX + Buffer.from(JSON.stringify(token), 'utf8').toString('base64');
}
...
function decodeToken(token: string): SessionToken {
  ...
  let raw = '';
  try {
    raw = Buffer.from(token.slice(TOKEN_PREFIX.length), 'base64').toString('utf8');
  } catch {
    throw new Error('zhujump: invalid session token');
  }
  ...
  return baseUrl ? { c: cookie, i: emailId, b: baseUrl } : { c: cookie, i: emailId };
}
...
async function ensureLoggedIn(baseUrl: string, cookie: string, username: string): Promise<void> {
...
async function loginAndCreate(instance: MoemailInstance): Promise<{ email: string; emailId: string; cookie: string }> {
  ...
  const loginResp = await fetchWithTimeout(`${baseUrl}/api/auth/callback/credentials?`, {
    method: 'POST',
    headers: {
      ...headers,
      Cookie: cookie,
  ...
  const body: Record<string, string | number> = { name: local, domain: instance.domain };
  if (instance.expiryTime !== null) {
    body.expiryTime = instance.expiryTime ?? DEFAULT_EXPIRY_TIME;
  }

  const generateResp = await fetchWithTimeout(`${baseUrl}/api/emails/generate`, {
    method: 'POST',
    headers: {
      ...headers,
      Cookie: cookie,
  ...
  const generateJson = (await generateResp.json()) as { id?: string; email?: string };
 ...
 */
export async function generateEmail(domain: string, channel: Channel): Promise<InternalEmailInfo> {
 ...
 */
export async function generateEmailForInstance(instance: MoemailInstance): Promise<InternalEmailInfo> {
  const baseUrl = instance.baseUrl.replace(/\/$/, '');
...
async function fetchMessageDetail(baseUrl: string, cookie: string, emailId: string, messageId: string): Promise<Record<string, unknown> | null> {
  const response = await fetchWithTimeout(`${baseUrl}/api/emails/${encodeURIComponent(emailId)}/${encodeURIComponent(messageId)}`, {
    headers: {
      ...jsonHeaders(baseUrl),
      Cookie: cookie,
    },
...
function hasMessageBody(row: { content?: string; html?: string }): boolean {
  return Boolean(String(row.content || '').trim() || String(row.html || '').trim());
}
...
export async function getEmails(token: string, email: string): Promise<Email[]> {
  ...
  const response = await fetchWithTimeout(`${baseUrl}/api/emails/${encodeURIComponent(session.i)}`, {
    headers: {
      ...jsonHeaders(baseUrl),
      Cookie: session.c,
    },
  ...
  for (const row of rows) {
    ...
    if (id && !hasMessageBody(row)) {
      const detail = await fetchMessageDetail(baseUrl, session.c, session.i, id);
      if (detail) {
        source = { ...row, ...detail };
      }
    }
