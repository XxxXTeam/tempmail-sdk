 ...
 */
export async function generateEmail(domain: string = DEFAULT_DOMAIN): Promise<InternalEmailInfo> {
  ...
  const data = await r2.json() as { success?: string; user?: string; time?: number };
  ...
  for (const item of mailList) {
    ...
    if (fid) {
      try {
        ...
        if (dr.ok) {
          const detail = await dr.json() as { mail?: string; success?: string };
