...
async function fetchDetail(uid: string): Promise<InboxesDetail | null> {
  try {
    return await fetchJSON<InboxesDetail>(`${API_BASE}/message/${encodeURIComponent(uid)}`);
  } catch {
    return null;
  }
}
...
export async function getEmails(email: string): Promise<Email[]> {
  ...
  const data = await fetchJSON<InboxResponse>(`${API_BASE}/inbox/${encodeURIComponent(address)}`);
