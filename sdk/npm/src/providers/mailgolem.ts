 ...
 */
async function fetchSession(existingCookie?: string): Promise<{ cookie: string; csrf: string }> {
...
export async function getEmails(email: string, token: string): Promise<Email[]> {
  ...
  const r = await fetchWithTimeout(`${BASE}/fetch-emails/${encodeURIComponent(addr)}`, {
    method: 'POST',
    headers: {
      ...PAGE_HEADERS,
