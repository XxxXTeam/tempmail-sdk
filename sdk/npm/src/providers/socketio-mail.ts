...
export function createSocketIoMailProvider(channel: Channel, defaultHost: string) {
  ...
  async function connectSocket(host: string): Promise<WebSocket> {
    ...
    throw lastError || new Error(`${channel}: websocket handshake failed`);
  }
  ...
  async function requestShortId(host: string): Promise<string> {
    ...
    return await new Promise<string>((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        reject(new Error(`${channel}: timed out waiting for shortid`));
      }, CONNECT_TIMEOUT_MS);
  ...
  async function getEmails(email: string): Promise<Email[]> {
