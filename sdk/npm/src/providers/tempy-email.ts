...
export async function generateEmail(domain: string | null = null, channel: Channel = CHANNEL): Promise<InternalEmailInfo> {
...
export async function getEmails(email: string): Promise<Email[]> {
  ...
  const response = await fetchWithTimeout(`${API_BASE}/mailbox/${encodeURIComponent(address)}/messages`, {
    headers: {
      Accept: 'application/json',
      'User-Agent':
        'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/146.0.0.0 Safari/537.36',
  ...
  const data = (await response.json()) as ListResponse | Record<string, unknown>;
  const rows = Array.isArray((data as ListResponse).messages)
    ? (data as ListResponse).messages || []
    : Array.isArray(data)
      ? (data as Record<string, unknown>[]) 
      : [];
  return rows.map((row: any) => normalizeEmail(flattenMessage(row, address), address));
}
