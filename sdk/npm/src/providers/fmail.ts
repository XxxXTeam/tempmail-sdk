...
export async function getEmails(email: string): Promise<Email[]> {
  ...
  for (const row of rows) {
    ...
    if (!token) {
      out.push(
        normalizeEmail(
          flattenMessage(row as Record<string, unknown>, email),
          email,
        ),
      );
      continue;
    ...
    try {
      const detail = await fetchJSON<EmailDetailResponse>(
        `${BASE_URL}/v1/email/${encodeURIComponent(token)}`,
      );
      out.push(
        normalizeEmail(
          flattenMessage(detail as Record<string, unknown>, email),
          email,
        ),
      );
    } catch {
