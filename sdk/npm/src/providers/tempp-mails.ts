/**
 * tempp-mails.com 临时邮箱渠道（Laravel + CSRF）
 *
 * 流程：
...
const CHANNEL: Channel = 'tempp-mails';
const BASE_URL = 'https://tempp-mails.com';
...
interface TemppMailsResponse {
  mailbox?: string;
  messages?: TemppMailsMessage[];
}
...
function decodeToken(token: string): { csrf: string; cookie: string } {
  ...
  try {
    ...
    throw new Error('tempp-mails: token 解码失败');
  }
  const obj = JSON.parse(raw) as { csrf?: string; cookie?: string };
  if (!obj.csrf) {
    throw new Error('tempp-mails: 缺少 CSRF token');
  }
...
export async function generateEmail(): Promise<InternalEmailInfo> {
  const r1 = await fetchWithTimeout(`${BASE_URL}/en`, { headers: browserHeaders() });
  if (!r1.ok) {
    throw new Error(`tempp-mails: 获取首页失败 HTTP ${r1.status}`);
  }
  ...
  if (!m?.[1]) {
    throw new Error('tempp-mails: 未能从首页提取 CSRF token');
  }
  const csrf = m[1];
  ...
  if (!r2.ok) {
    throw new Error(`tempp-mails: 获取邮箱失败 HTTP ${r2.status}`);
  }
  cookie = mergeCookieHeader(cookie, r2.headers);

  const data = (await r2.json()) as TemppMailsResponse;
  const mailbox = (data.mailbox ?? '').trim();
  if (!mailbox || !mailbox.includes('@')) {
    throw new Error(`tempp-mails: 邮箱地址无效: ${mailbox}`);
  }
...
export async function getEmails(token: string, email: string): Promise<Email[]> {
  ...
  if (!addr) {
    throw new Error('tempp-mails: 邮箱地址为空');
  }
  ...
  if (!r.ok) {
    throw new Error(`tempp-mails: 获取邮件列表失败 HTTP ${r.status}`);
  }

  const data = (await r.json()) as TemppMailsResponse;
...
async function fetchView(cookie: string, id: string): Promise<string> {
  ...
  const r = await fetchWithTimeout(`${BASE_URL}/view/${encodeURIComponent(id)}`, {
    headers: { ...browserHeaders(), Referer: `${BASE_URL}/en`, ...(cookie ? { Cookie: cookie } : {}) },
  });
  if (!r.ok) {
    return '';
  }
