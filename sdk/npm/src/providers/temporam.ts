...
export async function getEmails(email: string): Promise<Email[]> {
  const list = await getJson<EmailsResponse>(
    `/api/emails?email=${encodeURIComponent(email)}&limit=50`,
  );
  ...
  for (const row of rows) {
    ...
    try {
      const detail = await getJson<EmailDetailResponse>(
        `/api/emails/${encodeURIComponent(String(id))}`,
      );
