...
function createMirrorProvider(channel: Channel, apiBase: string) {
  async function generateEmail(): Promise<InternalEmailInfo> {
    const res = await fetchWithTimeout(`${apiBase}?f=get_email_address&lang=en`, { headers: HEADERS });
    if (!res.ok) throw new Error(`${channel}: generate failed: ${res.status}`);
    const data = await res.json() as { email_addr?: string; sid_token?: string; email_timestamp?: number };
    if (!data.email_addr || !data.sid_token) throw new Error(`${channel}: missing email_addr or sid_token`);
    return {
      channel,
  ...
  async function getEmails(token: string, email: string): Promise<Email[]> {
    const listRes = await fetchWithTimeout(
      `${apiBase}?f=check_email&seq=0&sid_token=${encodeURIComponent(token)}`, { headers: HEADERS });
    if (!listRes.ok) throw new Error(`${channel}: get emails failed: ${listRes.status}`);
