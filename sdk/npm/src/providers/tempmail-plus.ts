...
export async function getEmails(email: string): Promise<Email[]> {
  const listRes = await fetchWithTimeout(`${API_BASE}/?email=${encodeURIComponent(email)}&epin=`, { headers: HEADERS });
  if (!listRes.ok) throw new Error(`tempmail-plus: list failed: ${listRes.status}`);
    ...
    if (item.mail_id) {
      try {
        const dr = await fetchWithTimeout(
          `${API_BASE}/${item.mail_id}?email=${encodeURIComponent(email)}&epin=`, { headers: HEADERS });
        if (dr.ok) {
