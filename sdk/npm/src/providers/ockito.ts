...
async function refreshAccessToken(refreshToken: string): Promise<string> {
  const response = await fetchJSON<AccessTokenResponse>(`${BASE_URL}/grefresh`, {
    method: 'POST',
    headers: {
...
async function fetchBearerJSON<T>(
  ...
  const request = async (accessToken: string): Promise<{ status: number; json: T }> =>
    fetchJSON<T>(url, {
      ...init,
      headers: {
        ...(init?.headers || {}),
...
function flattenInboxRow(raw: Record<string, unknown>, recipient: string): Record<string, unknown> {
  return {
    id: raw.uid || raw.id || '',
...
function flattenMessage(raw: MailMessageResponse | Record<string, unknown>, recipient: string): Record<string, unknown> {
  const obj = (raw as MailMessageResponse)?.obj && typeof (raw as MailMessageResponse).obj === 'object'
...
export async function getEmails(token: string, email: string): Promise<Email[]> {
  ...
  for (const row of rows) {
    ...
    if (!uid) {
      out.push(normalizeEmail(flattenInboxRow(row as Record<string, unknown>, address), address));
      continue;
    }
