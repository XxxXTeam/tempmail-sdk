...
const GET_HEADERS = Object.fromEntries(
  Object.entries(DEFAULT_HEADERS).filter(
...
async function fetchEmailDetail(id: string): Promise<any | null> {
  const response = await fetchWithTimeout(
    `${BASE_URL}/emails/${encodeURIComponent(id)}`,
    {
      method: "GET",
      headers: GET_HEADERS,
    },
  ...
  return data && data.success && data.email && typeof data.email === "object"
    ? data.email
    : null;
}
...
export async function getEmails(
  ...
  const response = await fetchWithTimeout(
    `${BASE_URL}/mailboxes/${address}/emails`,
    {
      method: "GET",
      headers: GET_HEADERS,
    },
