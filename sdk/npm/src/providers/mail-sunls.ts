...
async function fetchDomains(): Promise<string[]> {
  ...
  const list = data.filter((x): x is string => typeof x === 'string' && x.trim().length > 0).map(d => d.trim());
  if (list.length === 0) {
...
export async function getEmails(email: string): Promise<Email[]> {
  ...
  for (const item of data) {
    ...
    if (mailId && !text && !html) {
      try {
        const detailRes = await fetchWithTimeout(`${BASE_URL}/api/fetch/${encodeURIComponent(String(mailId))}`, {
          method: 'GET',
          headers: HEADERS,
        });
        if (detailRes.ok) {
