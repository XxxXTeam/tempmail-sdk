...
async function initSession(): Promise<EmailnatorSession> {
  ...
  if (!cookie || !xsrf)
    throw new Error("emailnator: missing session cookie or XSRF token");
  return { xsrfToken: xsrf, cookie };
}

async function postJSON<T>(
  session: EmailnatorSession,
  path: string,
  body: unknown,
): Promise<T> {
...
async function postText(
  ...
  if (!response.ok) {
    throw new Error(
      `emailnator ${path}: ${response.status} ${text.slice(0, 160)}`,
    );
  }
  return text;
}
...
function decodeSession(token: string): EmailnatorSession {
  ...
  if (!data.cookie || !data.xsrfToken)
    throw new Error("emailnator: invalid session token");
  return { cookie: data.cookie, xsrfToken: data.xsrfToken };
}

export async function generateEmail(): Promise<InternalEmailInfo> {
  const session = await initSession();
  const data = await postJSON<GenerateResponse>(session, "/generate-email", {
    email: EMAIL_OPTIONS,
  });
  const email =
    data.email?.find(
...
export async function getEmails(
  ...
  const data = await postJSON<ListResponse>(session, "/message-list", {
    email,
  });
