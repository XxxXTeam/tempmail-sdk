...
async function postProxy(route: 'generate' | 'inbox', deviceId: string, body: unknown): Promise<ProxyPayload> {
  const ip = randomIP();
  ...
  if (!response.ok) {
    const reason = payload.detail || payload.error || payload.message || response.statusText;
...
export async function generateEmail(): Promise<InternalEmailInfo> {
  ...
  const payload = await postProxy('generate', deviceId, { variant: 'googlemail' });
...
export async function getEmails(deviceId: string, email: string): Promise<Email[]> {
  const payload = await postProxy('inbox', deviceId, { email });
  const messages = Array.isArray(payload.messages) ? payload.messages : [];
  return messages.map((raw: any) => normalizeEmail({
    ...raw,
