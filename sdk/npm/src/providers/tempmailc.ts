...
function flattenListMessage(raw: Record<string, unknown>, recipient: string): Record<string, unknown> {
  return {
    id: raw.id,
    from: raw.from,
    to: recipient,
...
async function fetchDetail(email: string, id: string): Promise<Record<string, unknown> | null> {
...
export async function getEmails(email: string): Promise<Email[]> {
  ...
  const response = await fetchWithTimeout(`${BASE_URL}/inbox?email=${encodeURIComponent(address)}`, {
    method: 'GET',
    headers: { Accept: 'application/json' },
  });
  if (!response.ok) {
