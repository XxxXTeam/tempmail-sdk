...
const CHANNEL: Channel = 'tempmailten';
const BASE_URL = 'https://tempmailten.com';
...
interface TempmailtenResponse {
  mailbox?: string;
  messages?: TempmailtenMessage[];
}
...
function decodeToken(token: string): { csrf: string; cookie: string } {
  ...
  try {
    ...
    throw new Error('tempmailten: token 解码失败');
  }
  const obj = JSON.parse(raw) as { csrf?: string; cookie?: string };
  if (!obj.csrf) {
    throw new Error('tempmailten: 缺少 CSRF token');
  }
...
export async function generateEmail(): Promise<InternalEmailInfo> {
  ...
  if (!r1.ok) {
    throw new Error(`tempmailten: 获取首页失败 HTTP ${r1.status}`);
  }
  ...
  if (!m?.[1]) {
    throw new Error('tempmailten: 未能从首页提取 CSRF token');
  }
  const csrf = m[1];
  ...
  if (!r2.ok) {
    throw new Error(`tempmailten: 获取邮箱失败 HTTP ${r2.status}`);
  }
  cookie = mergeCookieHeader(cookie, r2.headers);

  const data = (await r2.json()) as TempmailtenResponse;
  const mailbox = (data.mailbox ?? '').trim();
  if (!mailbox || !mailbox.includes('@')) {
    throw new Error(`tempmailten: 邮箱地址无效: ${mailbox}`);
  }
...
export async function getEmails(token: string, email: string): Promise<Email[]> {
  ...
  if (!addr) {
    throw new Error('tempmailten: 邮箱地址为空');
  }
  ...
  if (!r.ok) {
    throw new Error(`tempmailten: 获取邮件列表失败 HTTP ${r.status}`);
  }

  const data = (await r.json()) as TempmailtenResponse;
