...
function mapAttachments(raw?: MailforspamAttachment[]): Record<string, unknown>[] {
...
function flattenDetail(raw: MailforspamDetail, recipient: string): Record<string, unknown> {
  return {
    id: raw.id || '',
    from: raw.sender || '',
...
function flattenList(raw: MailforspamListMessage, recipient: string): Record<string, unknown> {
  return {
    id: raw.id || '',
    from: raw.sender || '',
    to: raw.receiver || recipient,
...
async function fetchMessage(id: string, email: string): Promise<MailforspamDetail | null> {
...
export async function generateEmail(domain?: string | null, channel: Channel = CHANNEL): Promise<InternalEmailInfo> {
  ...
  if (!response.ok) {
    throw new Error(`mailforspam: validate mailbox failed HTTP ${response.status}`);
  }
...
export async function getEmails(email: string): Promise<Email[]> {
  ...
  for (const item of list) {
    ...
    emails.push(normalizeEmail(detail ? flattenDetail(detail, address) : flattenList(item, address), address));
  }
  return emails;
}
