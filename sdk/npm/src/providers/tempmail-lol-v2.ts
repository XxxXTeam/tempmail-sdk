...
export async function generateEmail(): Promise<InternalEmailInfo> {
  ...
  if (!res.ok)
    throw new Error(`tempmail-lol-v2: generate failed: ${res.status}`);
  const data = (await res.json()) as { address?: string; token?: string };
  if (!data.address || !data.token)
    throw new Error("tempmail-lol-v2: missing address or token");
...
export async function getEmails(
  ...
  const res = await fetchWithTimeout(
    `${API_BASE}/auth/${encodeURIComponent(token)}`,
    { headers: HEADERS },
  );
  if (!res.ok)
    throw new Error(`tempmail-lol-v2: get emails failed: ${res.status}`);
