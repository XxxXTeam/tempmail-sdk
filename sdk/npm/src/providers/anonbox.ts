...
const MAIL_LINK_RE = /<a href="https:\/\/anonbox\.net\/([a-z0-9-]+)\/([^"]+)">https:\/\/anonbox\.net\/[^"]+<\/a>/i;
const DD_RE = /<dd([^>]*)>([\s\S]*?)<\/dd>/gis;
const DISPLAY_NONE_RE = /display\s*:\s*none/i;
const P_RE = /<p>([\s\S]*?)<\/p>/is;
const SPAN_RE = /<span\b[^>]*>[\s\S]*?<\/span>/gis;
const TAG_RE = /<[^>]+>/g;
const EXPIRES_RE = /Your mail address is valid until:<\/dt>\s*<dd><p>([^<]+)<\/p>/is;
...
function parseEnPage(html: string): { email: string; token: string; expiresAt?: string } {
  ...
  for (const match of html.matchAll(DD_RE)) {
    ...
    const domain = display.slice(at + 1).trim().toLowerCase();
...
function decodeQuotedPrintable(body: string, headerBlock: string): string {
  ...
  return new TextDecoder('utf-8').decode(new Uint8Array(bytes)).replace(/\r?\n+$/, '');
}

function parseHeaders(lines: string[]): { headers: Record<string, string>; bodyIndex: number } {
...
function mboxBlockToRaw(block: string, recipient: string): Record<string, unknown> {
  ...
  if (contentType.includes('multipart/')) {
    ...
    if (boundaryMatch) {
      ...
      for (const partRaw of parts) {
        ...
        const pct = /^content-type:\s*([^\s;]+)/im.exec(ph)?.[1]?.trim().toLowerCase() || '';
        if (pct === 'text/plain') {
