...
export async function generateEmail(): Promise<InternalEmailInfo> {
  ...
  const res = await fetchWithTimeout(`${API_BASE}/inbox/${encodeURIComponent(local)}`, {
...
export async function getEmails(email: string): Promise<Email[]> {
  ...
  if (!res.ok) throw new Error(`harakirimail: get emails failed: ${res.status}`);
  ...
  for (const item of list) {
    ...
    if (item._id && !body && !html) {
      try {
        const dr = await fetchWithTimeout(`${API_BASE}/email/${encodeURIComponent(item._id)}`, {
          headers: { 'Accept': 'application/json', 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' },
        });
        if (dr.ok) {
    ...
    out.push(normalizeEmail({
      ...
      text: typeof body === 'string' ? body.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim() : '',
      html: html || body || '',
